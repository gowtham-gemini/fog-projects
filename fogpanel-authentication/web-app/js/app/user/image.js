    ...
    'populateValues': function() {
        ...
        var dataLayout = [[
                ...
                {'name': translator.common.name, 'field': 'name', 'width': '200px', 'datatype':'string',  'autoComplete': 'true', 'formatter': function(data) {
                        var viewTag = "<a href='#/user/image/view/"+data.id+"' title='"+translator.common.view+"'>"+data.name+"</a>";
                        return viewTag;
                }},
                 ...
                 {'name': translator.common.action, 'field': 'action',
                     'formatter': function(data) { 
                        ...
                        if(data.action == true) {
                            ...
                            //chaeck if image is snapshot
                            if(data.isVMSnapshot == true) {
...
var EditImage = {
    ...
    'init': function(id) {        
        ...
        var getImageRestStore = new JsonRest({
         target: core.getContextPath() + "/api/image/listByAccount"
        });
        var userImage = "true";
    ...
    'view': function(id) {
        ...
        viewRest.query({referenceId: id}).then(function(data) {
            console.log(data)
            
            dojo.forEach(data, function(resultData) {
                dojo.byId("imageNameInfo").innerHTML = resultData.name;
                dojo.byId("imageIdInfo").innerHTML = resultData.id;
                ...
                dojo.byId("imageBillingTypeInfo").innerHTML = resultData.billingType;
                dojo.byId("imageHypervisorInfo").innerHTML = resultData.hypervisor;
                dojo.byId("imageTypeInfo").innerHTML = resultData.imageType;
                dojo.byId("imageIsPublicInfo").innerHTML = (resultData.isPublic) == true ? "Yes" : "No";
                dojo.byId("imageIsVMSnapshotInfo").innerHTML = (resultData.isVMSnapshot) == true ? "Yes" : "No";
                dojo.byId("imageRootDeviceNameInfo").innerHTML = resultData.rootDeviceName;
                dojo.byId("imageRootDeviceTypeInfo").innerHTML = resultData.rootDeviceType;
                dojo.byId("imageVirtualizationTypeInfo").innerHTML = resultData.virtualizationType;
                dojo.byId("imageCreatedInfo").innerHTML = (resultData.createdOn) == undefined ? "" : resultData.createdOn;
            });
        });    
    }
