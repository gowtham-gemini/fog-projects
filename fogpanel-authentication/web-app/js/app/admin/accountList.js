...
var Summary = {
    ...
    viewUser : function(id) {
        ...
        var viewDetails = new JsonRest({
            target:core.getContextPath()+"/api/user/view"
        })
        
        viewDetails.query({id:id}).then(function(data){
           dojo.forEach(data, function(userDetails) {
                dojo.byId("userNameInfo").innerHTML = userDetails.name
                dojo.byId("userFirstNameInfo").innerHTML = userDetails.firstName
                dojo.byId("userLastNameInfo").innerHTML = userDetails.lastName
                dojo.byId("userAccountLockedInfo").innerHTML = (userDetails.accountLocked == true) ? "Yes" : "No"
                dojo.byId("userStateInfo").innerHTML = (userDetails.enabled == true) ? "<div class='client-stat-active overflowLabel'>"+translator.common.account.status.ACTIVE+"</div>" : "<div class='client-stat-canceled overflowLabel'>"+translator.common.account.status.DISABLED+"</div>"
                dojo.byId("userEmailInfo").innerHTML = userDetails.email
            });
        })
    },
...
var CurrentInvoiceInfo = {
    ...
    populateValues: function() {
        ...
        dojo.byId("userInvoiceList").innerHTML = "<img src='images/vmload.gif' alt='"+translator.common.loader.loading+"' height='36' width='10%'/> </br> <p>"+translator.common.loader.loadingPleaseWait+"</p>";
        var invoiceData = {
            items: []
        };
...
//    _accountGrid: "",
    ...
    init: function() {
        this._accountListRestStore = new JsonRest({
            target: core.getContextPath()+"/api/user/userList"
        });
    ...
    populateValues: function() {
        ...
        dojo.byId("accountGrid").innerHTML = "<img src='images/vmload.gif' alt='"+translator.common.loader.loading+"' height='36' width='10%'/> </br> <p>"+translator.common.loader.loadingPleaseWait+"</p>"; 
...
//                },
                {'name': translator.user.userName, 'field': 'userName', 'width': '10%', datatype:"string",  autoComplete: true, 'formatter': function(data) {
                        var node = "<a href='#/admin/users/view/" + data.id + "'>" + data.username + "</a>";
                        return node;
                    }}, 
