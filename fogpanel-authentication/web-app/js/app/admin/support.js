    ...
    'populateValues': function() {
        ...
        var gridLayout = [[
                ...
                {'name': translator.common.action, 'field': 'action', 'width': '100%', 'formatter': function(data) {
                      var html = "<a title='"+translator.common.edit+"' onclick='AddDepartment.viewDep(\"" + data.id + "\", \""+ data.name +"\")'><i class='icon-edit'></i></a>";                                                                                                  
                        return  html;
                  }
              }
            ]
...
//                        location.reload();
    ...
    'viewDep': function(id, name) {
        dojo.byId("depId").value = id;
        dijit.byId("departmentName").setValue(name);
