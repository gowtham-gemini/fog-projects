        ...
        function (dojo, i18n, translator, dijit, domClass, domConstruct, JsonRest, query, registry, FilteringSelect, Select,
            ...
            },
            {
                ...
                "sync": action(function () {
                    core.ui.loadTemplate("sync", core.ui.getContentId());
                }),
                "cloudEngine": action(function () {

                    var currentPage = dijit.byId("adminCloudEngineListForm");
                    if (currentPage) {
                        dijit.byId("adminCloudEngineListForm").destroyRecursive();
                    }
                    if(dijit.byId("cloudEngineDeleteDialog")) {
                        dijit.byId("cloudEngineDeleteDialog").destroyRecursive();
                    }
                    if(dijit.byId("adminCloudEngineEditForm")) {
                        dijit.byId("adminCloudEngineEditForm").destroyRecursive();
                    }
                    if (dijit.byId("engineTypeListWidget")) {
                        dijit.byId("engineTypeListWidget").destroyRecursive();
                    }
                    core.ui.loadTemplate("cloudEngineList", core.ui.getContentId());
                    CloudEngine.populateValues();
                }),
                "addCloudEngine": action(function () {

                    var currentPage = dijit.byId("adminCloudEngineAddForm");
                    if (currentPage) {
                        dijit.byId("adminCloudEngineAddForm").destroyRecursive();
                    }
                    core.ui.loadTemplate("addCloudEngine", core.ui.getContentId());
                    CloudEngine.init();
                }),
                ...
                "aws": action(function () {
                    core.ui.loadTemplate("aws", core.ui.getContentId());
                }),
                "zoneConfig": action(function () {
                    if (dijit.byId("zoneListForm")) {
                        dijit.byId("zoneListForm").destroyRecursive();
                    }
                    core.ui.loadTemplate("zoneList", core.ui.getContentId());
                    ZoneConfig.populateValues()
                }),
                ...
                "importData": action(function () {
                    ...
                    core.ui.loadTemplate("importDataFromAws", core.ui.getContentId());
                    OpenstackConfig.testInResourcesImportPage();
//            ImportDataFromOpenstack.init();
        }),
        "viewCloudEngine":action(function(id){
            core.ui.loadTemplate("viewCloudEngine", core.ui.getContentId());
                    CloudEngine.view(id)
        }),
        "editCloudEngine": action(function(id){
            var currentPage = dijit.byId("adminCloudEngineEditForm");
            if (dijit.byId("adminCloudEngineAddForm")) {
                dijit.byId("adminCloudEngineAddForm").destroyRecursive();
            }
            ...
            core.ui.loadTemplate("editCloudEngine", core.ui.getContentId());
            CloudEngine.edit(id)
        })
...
var CloudEngine = {
    'init': function () {
        var engineTypeRestStore = new JsonRest({
            target: core.getContextPath() + "/api/cloudEngine/hardCodedType"
        });
        ...
        dojo.byId("engineTypeList").innerHTML = translator.common.loading;
        var engineTypeList = new ItemFileWriteStore({data: engineTypeOptions});
        engineTypeRestStore.query().then(function (data) {

            console.log("data" + data);

            dojo.forEach(data, function (el) {
                console.log("el" + el);
                engineTypeList.newItem({id: el.id, name: el.name});
            });
            dojo.byId("engineTypeList").innerHTML = "";
            var engineTypeWidget = new FilteringSelect({
                store: engineTypeList,
                id: "engineTypeListWidget",
                required: true,
                placeHolder: translator.common.selectCloudEngineType,
                onChange: function () {
                }
            });
            engineTypeWidget.placeAt("engineTypeList");
            engineTypeWidget.startup();
        });
    },
    
    'cancel': function() {
        core.router.go("#/admin/settings/cloudEngine");
    },
    
    'add': function () {
        dojo.byId("admincloudEngineAddButtonDiv").style.display = "block";
        var status = true;
        var pageNode = dojo.byId("adminCloudEngineContent");
        ...
        if (firstNode) {
            ...
            var cloudEngineStore = new JsonRest({
                target: core.getContextPath() + "/api/cloudEngine/"
            });

            var name = dijit.byId("cloudEngineName").value;
            var engineType = dijit.byId("engineTypeListWidget").value;
            var cloudEngineUrl = dijit.byId("cloudEngineUrl").value;


            dojo.byId("admincloudEngineLoader").style.display = "none";
            dojo.byId("admincloudEngineAddButtonDiv").style.display = "block";

            cloudEngineStore.add({
                name: name,
                engineType: engineType,
                cloudEngineUrl: cloudEngineUrl
            }).then(function (data) {
                ...
                if (data[0].result === "OK") {
                    ...
                    dojo.byId("admincloudEngineAddButtonDiv").style.display = "none";
                    dojo.byId("admincloudEngineLoader").style.display = "block";
                    core.router.go("#/admin/settings/cloudEngine");
                } else if(data[0].error === "Already Exist") {
                    registry.byId("appToaster").setContent(translator.common.message.alreadyExist, "error");
                    registry.byId("appToaster").show();
//                    dijit.byId("addCloudEngineLoader").hide();
                    dojo.byId("admincloudEngineAddButtonDiv").style.display="none";
                    dojo.byId("admincloudEngineLoader").style.display="block";
                } else {
...
//                    dijit.byId("addCloudEngineLoader").hide();
    ...
    view: function (id) {
        ...
        var cloudEngineViewStore = new JsonRest({
            target: core.getContextPath() + "/api/cloudEngine/view"
        });
        

        cloudEngineViewStore.query({engineId: id}).then(function(data){
           dojo.forEach(data, function(resultData) {
                dojo.byId("cloudEngineNameInfo").innerHTML = resultData.name
                dojo.byId("cloudEngineTypeInfo").innerHTML = resultData.type
                dojo.byId("cloudEngineUrlInfo").innerHTML = resultData.url
                dojo.byId("CloudEngineStatusInfo").innerHTML = resultData.status
                dojo.byId("CloudEngineIdInfo").innerHTML = resultData.engineId
           });
...
//                {'name': "ID", 'field': 'id', 'width': '200px', 'datatype': 'string', 'autoComplete': 'true'},
                ...
                {'name': translator.common.name, 'field': 'name', 'width': '10%', 'datatype': 'string', 'autoComplete': 'true', 'formatter': function(data) {
                        ...
                        var viewTag = "<a href='#/admin/settings/viewCloudEngine/" + data.id + "' title='" + translator.common.edit + "'>" + data.name + "</a>";
                        return viewTag;
                    }},
                {'name': translator.common.engineId, 'field': 'engineId', 'width': '10%', 'datatype': 'string', 'autoComplete': 'true'},
                {'name': translator.common.url, 'field': 'url', 'width': '10%', 'datatype': 'string', 'autoComplete': 'true'},
                {'name': translator.common.cloudEngineStatus, 'field': 'status', 'width': '10%', 'datatype': 'string', 'autoComplete': 'true'},
    ...
    //                                NetworkInfo.showEditNetworkDialog(data.referenceId);
                        ...
                        var menuItem1 = new MenuItem({
                            ...
                            onClick: function() {
                                core.router.go("#/admin/settings/viewCloudEngine/" + data.id);
                            }
                        });
                        ...
                        var menuItem2 = new MenuItem({
                            ...
                            onClick: function() {
                                core.router.go("#/admin/settings/editCloudEngine/" + data.id + "");
//                                NetworkInfo.showEditNetworkDialog(data.referenceId);
                            }
                        });
                        ...
                        var menuItem3 = new MenuItem({
                            ...
                            onClick: function() {
                                CloudEngine.deleteCloudEngineConfirmAlert(data.id);
                            }
                        });
                    ...
                    }, 'width': '10%', 'datatype': 'string', 'autoComplete': 'true'}
            ]
        ];
        ...
        restStore.query().then(function (data) {
            if (data.length == 0 || data == undefined || data == 'undefined' || data == '' || data == " ") {
                dojo.byId("adminCloudEngineList").innerHTML = "";
                dojo.byId("noCloudEngineMessageBox").style.display = "block";

            } else {

                dojo.byId("noCloudEngineMessageBox").style.display = "none";
                dojo.forEach(data, function (resultData) {
                    dataList.newItem({
                        name: resultData,
                        engineId: resultData.engineId,
                        url: resultData.url,
...
//                        id: 'computDataGrid',
                ...
                dataGrid.placeAt("adminCloudEngineList");
                dataGrid.startup();
                dataGrid.update();
    ...
    deleteCloudEngineConfirmAlert : function(id) {
       dojo.byId("currentCloudEngineId").value = id
       dijit.byId("cloudEngineDeleteDialog").show()
       
//       var cloudEngineDeleteStore = new JsonRest({
//            target: core.getContextPath() + "/api/cloudEngine/delete"
//        })
//       
//       cloudEngineDeleteStore.query({id:id}).then(function(data){
//           console.log(data)
//       })
    },
    
    deleteCloudEngine : function() {
        var id = dojo.byId("currentCloudEngineId").value
...
//       
       cloudEngineDeleteStore.query({id:id}).then(function(data){
           if(data[0].result === "OK") {
               CloudEngine.populateValues();
                registry.byId("appToaster").setContent(translator.common.message.deletedSuccess, "error");
                registry.byId("appToaster").show();
           } else {
                registry.byId("appToaster").setContent(translator.common.message.cannotBeDeleted, "error");
                registry.byId("appToaster").show();
           }
           
       })
       dijit.byId("cloudEngineDeleteDialog").hide()
    },
    
    
    
    closeDeleteCloudEngineDialog:function() {
            dijit.byId("cloudEngineDeleteDialog").hide()
    },
...
//            }
        ...
        var cloudEngineEditStore = new JsonRest({
            target: core.getContextPath() + "/api/cloudEngine/edit"
        })
        
        var cloudEngineNameId = dijit.byId("cloudEngineNameId");
        var cloudEngineUrlId = dijit.byId("cloudEngineUrlId");
...
//        dojo.byId("engineTypeList").innerHTML = translator.common.loading;
        dojo.byId("currentCloudEngineId").value = data
        var engineTypeList = new ItemFileWriteStore({data: engineTypeOptions});
        cloudEngineEditStore.query({id:data}).then(function(data){
            
            cloudEngineNameId.setValue(data[0].name);
            cloudEngineUrlId.setValue(data[0].url);
            dojo.forEach(data[0].type,function(resultData){
                engineTypeList.newItem({id: resultData.id, name: resultData.name});
            });
...
//                engineTypeList.newItem({id: el.id, name: el.name});
            ...
            engineTypeWidget.placeAt("engineTypeList");
            engineTypeWidget.startup();
            engineTypeWidget.set("value", data[0].engineId);
    ...
    update : function() {
        var cloudEngineNameId = dijit.byId("cloudEngineNameId").value;
        var cloudEngineType = dijit.byId("engineTypeListWidget").value
        var cloudEngineUrlId = dijit.byId("cloudEngineUrlId").value;
        var currentCloudEngineId = dojo.byId("currentCloudEngineId").value
        
        var updateCloudEngineRestStore = new JsonRest({
            target: core.getContextPath() + "/api/cloudEngine/update/"
        });
        
        updateCloudEngineRestStore.add({
            id:currentCloudEngineId,
            type:cloudEngineType,
            url:cloudEngineUrlId,
            name:cloudEngineNameId
        }).then(function(data){
            if(data[0].result === "OK") {
    ...
    'verify': function(id) {
        var engineRest = new JsonRest({
            target: core.getContextPath() + "/api/cloudEngine/verify"
        });
        var engineId = "";
        var cloudUrl; 
        var applicationURL = "";
        var verify = false;
        engineRest.query({cloudEngineId: id}).then(function(data) {
            dojo.forEach(data, function(resultData) {
            ...
            if(verify == true) {
                var changeStatusRest = new JsonRest({
                    target: core.getContextPath() + "/api/cloudEngine/changeStatus"
                });

                changeStatusRest.query({cloudEngineId: id}).then(function(data) {
                    dojo.forEach(data, function(resultData) {
                        if(resultData.result == "OK") {
                            verify = true;
                            registry.byId("appToaster").setContent("Your cloud engine is verfied successfully", "message");
                            registry.byId("appToaster").show();
                            CloudEngine.populateValues();
                        } else {
                            registry.byId("appToaster").setContent("Please check the cloud engine url", "error");
                            registry.byId("appToaster").show();
                        }
                    });
                });
...
var ZoneConfig = {
    'populateValues': function () {
        var awsZonesList = new JsonRest({
            target: core.getContextPath() + "/api/zone"
        })
        dojo.byId("zoneListGrid").innerHTML = "<img src='images/vmload.gif' alt='Loading' height='36' width='100'/> </br> <p>" + translator.common.loader.loading + "</p>";
        var gridData = {
            items: []
        };
        ...
        var gridLayout = [[
                ...
                {'name': translator.common.available, 'field': 'available', 'width': '10%', 'formatter': function (data) {
                        var html
                        if (data === true) {
                            html = "<span class='fog_cost_blue'>Yes</span>";
                        } else {
                            html = "<span class='fog_cost_red overflowLabel'>No</span>";
                        }
                        return html;
                    }
                }
        ...
        awsZonesList.query().then(function (data) {

            if (data.length === 0) {
                dojo.byId("zoneListGrid").innerHTML = "<div class='alert alert-info hide' style='display: block;'>\n\
                   <i class='icon-exclamation-sign'></i>" + translator.common.message.noZone + " ! " + " <a href='#/admin/settings/importData'> Click here to Pull Zones from AWS</a></div>";
            } else {
                dojo.forEach(data, function (el) {
                ...
                dojo.byId("zoneListGrid").innerHTML = "";
                ...
                availabilityZoneGrid.placeAt("zoneListGrid");
...
var RegionConfig = {
    ...
    'populateValues': function () {
        ...
        var gridLayout = [
            [
                {'name': translator.common.regionName, 'field': 'name', 'width': '100%'},
                {'name': translator.common.endPoint, 'field': 'endPoint', 'width': '100%'},
            ]
        ];
        ...
        regionListRestStore.query().then(function (data) {
            ...
            if (data.length == 0) {
                dojo.byId("regionListGrid").innerHTML = "<div class='alert alert-info hide' style='display: block;'>\n\
                   <i class='icon-exclamation-sign'></i>" + translator.common.message.noRegionAdded + " ! " + "<a href='#/admin/settings/importData'> Click here to Pull Regions from AWS</a></div>";
            } else {
    ...
    pullLdapData: function () {
        ...
        pullFromRestStore.query().then(function (data) {                        
            ...
            dojo.forEach(data, function (resultData) {
                ...
                if (resultData === "OK") {                    
                    registry.byId("appToaster").setContent("All users pulled success fully", "message");
                    registry.byId("appToaster").show();
...
window.syncLDAPData = syncLDAPData;
