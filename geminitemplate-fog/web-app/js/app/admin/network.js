...
//        accountList.newItem({id: "", name: ""});
    ...
    'update' : function() {
        ...
        var pageNode = dojo.byId("adminEditNetworkFormPage");
        var widgets = dijit.registry.findWidgets(pageNode);
        var firstNode = "";
        dojo.forEach(widgets, function (el) {
            if (el.validate && !el.validate()) {
        ...
        if (firstNode) {
            ...
            dijit.byId("adminNetworkUpdateLoader").show();
            var name = dijit.byId("adminEditNetworkName").value;
            var id = dijit.byId("adminEditNetworkId").value;
            var isAdminState = true;
            if(dijit.byId("adminEditNetworkState").checked == false) {
                isAdminState = false;
            }
    ...
    'showEditNetworkDialog' : function(id) {
        ...
        viewNetworkRestStore.query({referenceId : id}).then(function (data) {
            dojo.forEach(data, function (resultData) {
                ...
                dijit.byId("adminEditNetworkName").setValue(resultData.name);
                dijit.byId("adminEditNetworkId").setValue(resultData.referenceId);
                if(resultData.isAdminState == true) {
                    dijit.byId("adminEditNetworkState").checked == true;
                } else {
                    dijit.byId("adminEditNetworkState").checked == false;
                }
            });
        });
    },
