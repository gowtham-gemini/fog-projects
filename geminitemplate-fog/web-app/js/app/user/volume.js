...
var ListVolume = {
    ...
    'init': function() {
        ...
        if(dijit.byId("VMInstanceListForAttachment")) {
            dijit.byId("VMInstanceListForAttachment").destroyRecursive();                                
        }
...
//               dojo.byId("instanceListForAttachment").innerHTML = translator.common.noInstanceCreated;
                ListVolume._instanceListLength = 0;
            } else {                            
                ListVolume._instanceListLength = 1;
        ...
        var instanceWidget = new FilteringSelect({
            store : instanceList,
            id: "VMInstanceListForAttachment",
            placeHolder: translator.common.message.selectInstance
        });
        instanceWidget.placeAt("instanceListForAttachment");
            ...
            [
                ...
                {'name': translator.user.grid.storage.layout.attachedTo, 'field': 'attachedTo', 'width': '100px', datatype: "string", autoComplete: true,'formatter': function(data) {
                    ...
                    if(data.instanceName != null) {
                        return "<a href='#/user/server/viewServer/" + data.instanceReferenceId + "' title='" + translator.common.view + "'>" + data.instanceName + "</a>";
                    } else {
                        return "-";
                    }
                ...
                {'name': translator.user.grid.storage.layout.action, 'field': 'action', 'formatter': function(data, rowIndex) {
                        ...
                        if(ListVolume._instanceListLength == 0) {
                            
                        } else {
                        
                            if(data.instanceReferenceId) {

                                var menuItem2 = new MenuItem({
                                    label: translator.common.detachFromInstance,
                                    onClick: function() {
                                        ListVolume.detachVolumeAlertDialog(data.referenceId,data.instanceReferenceId);
                                    }
                                });
                                ...
                                var menuItem5 = new MenuItem({
                                    label: translator.common.attachToInstance,
                                    onClick: function() {
                                        ListVolume.showAttachToInstance(data.referenceId);
                                    }
                                });
                        ...
                        if(data.status !== "error") {
                            var menuItem3 = new MenuItem({
                                ...
                                onClick: function() {
                                    ListVolume.editVolume(data.referenceId);
                                }
                            });
                        ...
                        var menuItem4 = new MenuItem({
                            label: "Delete",
                            onClick: function() {
                                ListVolume.getDeleteConformation(data.referenceId);
                            }
                        });
            ...
            if (data.length === 0 || data === "undefined" || data === undefined) {
                ...
                dojo.forEach(data, function(instanceData) {
                    gridDataList.newItem({
                        id: instanceData.referenceId,
                        name: instanceData,
                        description: instanceData.description == "" ? "-" : instanceData.description,
                        size: instanceData.size,
                        status: instanceData.status,
                        volumeType: instanceData.volumeType ==  null ? "-" : instanceData.volumeType,
                        attachedTo: instanceData,
//                        instanceReferenceId: instanceData.instanceReferenceId != null || instanceData.instanceReferenceId != "null" ? instanceData.instanceReferenceId : "",
                        availabilityZone: instanceData.availabilityZone,
                        action: instanceData
                    });
                });
                dojo.byId("volumeListGrid").innerHTML = "";
