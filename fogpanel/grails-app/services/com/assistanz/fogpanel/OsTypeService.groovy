    ...
    def list() {
        try {  
            ...
            for(Iterator<OsCategoryResponse> iter = osCategoryResponse.osCategories.iterator(); iter.hasNext();) {
                ...
                OsCategory osCategory = OsCategory.findByOsCategoryId(categoryData.id);
                if (!osCategory) {
                    osCategory = new OsCategory();                    
                    osCategory.osCategoryId = categoryData.id;
                    osCategory.name = categoryData.name; 
                    for (Entry<String, String> entry : BASE_OS.entrySet()) {
                        if(entry.getKey() == categoryData.name) {
            ...
            for(Iterator<OsTypesResponse> iter = response.osTypes.iterator(); iter.hasNext();) {
                ...
                OsType osType = OsType.findByReferenceOsTypeId(data.id);
                if (!osType) {
                    osType  = new OsType();                    
                    osType.referenceOsTypeId = data.id;
                    osType.name = data.description; 
