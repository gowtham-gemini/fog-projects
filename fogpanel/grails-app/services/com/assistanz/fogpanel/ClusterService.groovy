    ...
    def list(String referencePodId) {
        try {
           ...
           for(Iterator<ClusterResponse> clusterIter = response.clusters.iterator(); clusterIter.hasNext();) {
               ...
               Cluster cluster = Cluster.findByClusterReferenceId(clusterData.id);
               if (!cluster) {
                   cluster = new Cluster()
                   cluster.clusterReferenceId = clusterData.id
                   cluster.name = clusterData.name
                   cluster.description = clusterData.name
                   cluster.zone = Zone.findByReferenceZoneId(clusterData.zoneId)
                   cluster.pod = Pod.findByPodReferenceId(clusterData.podId)
