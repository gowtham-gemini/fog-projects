...
//        }
        ...
        usageItem.put("defaultDomainConfig", Domain.findByIsDefault(true) ? "true" : "false");   
    ...
    def sendUsageFailedMail(usageFailureAccounts) {    
        ...
        log.user = User.findByUsername(log.account.accountIdentifier)
        log.date = time
    ...
    def calcVMSnapshotUsageForAllAccount(currentAccount) {
        ...
        def accountList;
        if(currentAccount) {
            accountList = Account.findAllWhere(accountIdentifier:currentAccount.accountIdentifier); 
        } else {
            // get all aviliable accounts. 
...
//                throw ex;
    ...
    private addUsageRecord(offeringUsage, data, startTimestamp, endTimestamp) {
        try {  
            ...
            def offering;
            def invoiceItem
            def invoice = Invoice.findWhere(account: Account.findByAccountIdentifier(data.account), status: InvoiceStatus.values()[6])
            ...
            offeringUsage.virtualMachineId = data.virtualMachineId;
            offeringUsage.virtualMachineName = data.name;
            offeringUsage.templateId = data.templateId;
            offeringUsage.account = Account.findByAccountIdentifier(data.account); 
            offeringUsage.domain = Account.findByAccountIdentifier(data.account).domain;
            ...
            if(data.usageType == "1") {
               ...
               invoiceItem = vms[0]
                ...
                def size = Double.parseDouble(data.size)/1073741824;
                if(offering) {
                    if(usageCalcType == "ACTUAL") {
