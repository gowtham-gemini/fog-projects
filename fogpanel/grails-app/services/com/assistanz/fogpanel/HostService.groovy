...
class HostService {
    ...
    def list(String clusterReferenceId) {
        ...
        for(Iterator<HostResponse> hostIter = response.hosts.iterator(); hostIter.hasNext();) {
            ...
            Double memory = Double.parseDouble(hostData.memoryTotal);
            Host host = Host.findByHostReferenceId(hostData.id);
            if (!host) {
                host = new Host()
                host.hostReferenceId = hostData.id
                host.name = hostData.name
                host.description = hostData.name
                host.cpuNumberTotal =  Integer.parseInt(hostData.cpuNumber) 
                host.coreCpuSpeedTotal =  Double.parseDouble(hostData.cpuSpeed) 
                host.memoryTotal = memory/1048576;
                host.hostTag = hostData.tags
                host.zone = Zone.findByReferenceZoneId(hostData.zoneId)
                host.pod = Pod.findByPodReferenceId(hostData.podId)
                host.cluster = Cluster.findByClusterReferenceId(hostData.clusterId)
    ...
    def getHostCapacity(String hostId) {
        ...
        for(Iterator<HostResponse> hostIter = response.hosts.iterator(); hostIter.hasNext();) {
            def hostData = hostIter.next();
            Double memory = Double.parseDouble(hostData.memoryTotal);
            Double cpuSpeed = Double.parseDouble(hostData.cpuSpeed);
            Double cpuNumber =  Integer.parseInt(hostData.cpuNumber);
            Double memoryUsed;
...
//            Console.print("hostData.hostMemoryUsed" + hostData.hostMemoryUsed)
            if(hostData.memoryUsed == null || hostData.memoryUsed == "null") {
                memoryUsed =  0;
            } else {
               memoryUsed =  Double.parseDouble(hostData.memoryUsed); 
            }
            if(hostData.memoryAllocated == null || hostData.memoryAllocated == "null") {
                memoryAllocated =  0;
            } else {
               memoryAllocated =  Double.parseDouble(hostData.memoryAllocated);
            }
            if(hostData.networKbsRead == null || hostData.networKbsRead == "null") {
                networkkbsRead =  0;
            } else {
                networkkbsRead =  Double.parseDouble(hostData.networKbsRead);
            }
            if(hostData.networKbsWrite == null || hostData.networKbsWrite == "null") {
                networkkbsWrite =  0;
            } else {
                networkkbsWrite =  Double.parseDouble(hostData.networKbsWrite);
            }
            HashMap<String,String> host = new HashMap<String,String>();    
            host.put("totalCpu", cpuNumber + " X " + cpuSpeed/1000 + " Ghz");
            host.put("memory",  Math.round((memory/1073741824) *100)/100  + " GB");
            host.put("cpuAllocated", hostData.cpuAllocated);
            host.put("cpuUsed", hostData.cpuUsed);
