...
class ZoneService {
    ...
    def list(advanced) {
        ...
        try {
            ...
            if(role.iterator().next() == "ROLE_ADMIN" ) {
                Account account = Account.findByAccountIdentifier(user.username);
                if(account.domain == null || account.domain == "" || account.domain == "null" || account.domain.name == "dummy") {
                    account.domain = Domain.findByLevel(0);
                    account.save(flush: true);
                    
                    user.domain = Domain.findByLevel(0);
    ...
    def addDefaultNetwork(zoneId) {
        ...
        for(Iterator <NetworkResponse> iter = response.networks.iterator(); iter.hasNext();) {
            ...
            if(data.name == "defaultGuestNetwork" || data.name == "guestNetworkForBasicZone") {
    ...
    def listAlerts() {
        ...
        try {
            ...
            if(response.alerts.size() > 0) {
                ...
                cloudStackUpDate.value = response.alerts[0].sent
                cloudStackUpDate.save(flush: true);

                return response.alerts[0].sent; 
            }
    ...
    def pullZoneFromCloudStack(jobid) {
...
//                    Zone zone = Zone.findByReferenceZoneId(data.zoneId);
                if(licenseValidationService.getProduct() == "FOG_BASIC"  && data.networkType == "Basic") { // Changed networkType as new api standard
                    Zone zone = Zone.findByReferenceZoneId(data.id);
                    if (!zone) {                        
...
//                        hypervisorService.get(new String(data.zoneId));
                        ...
                        addZone(data.id, data.name, data.networkType, data.name, data.name) // Changed networkType as new api standard
                        hypervisorService.get(new String(data.id)); // Changed zoneId  as id in  new api standard
//                        domainService.list();
                        osTypeService.list();
                        listAlerts();                                                
                    }
...
//                    Zone zone = Zone.findByReferenceZoneId(data.zoneId);
                    Zone zone = Zone.findByReferenceZoneId(data.id); // Changed zoneId  as id in  new api standard
                    if (!zone) {
//                        addZone(data.zoneId, data.zoneName, data.zoneNetworkType, data.zoneName, data.zoneName)
//                        hypervisorService.get(new String(data.zoneId));
                        addZone(data.id, data.name, data.networkType, data.name, data.name) // Changed zoneName  as name, zoneNetworkType as  networkType in  new api standard
                        hypervisorService.get(new String(data.id)); // Changed zoneId  as id in  new api standard
//                        domainService.list();
                        osTypeService.list();
                        listAlerts();
            ...
            //networkOfferService.pullNetworkFromZone(null) 
