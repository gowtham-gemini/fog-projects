    ...
    def list(String referenceZoneId) {
        try { 
            ...
            for(Iterator<PodResponse> podIter = response.pods.iterator(); podIter.hasNext();) {
                ...
                Pod pod = Pod.findByPodReferenceId(podData.id);
                if (!pod) {
                    pod = new Pod()
                    pod.podReferenceId = podData.id
                    pod.name = podData.name
                    pod.description = podData.name
                    pod.zone = Zone.findByReferenceZoneId(podData.zoneId) 
                    pod.save(flush: true);
                }
             }
