    ...
    populateValues : function() {
        this._currentAccRestStore.query().then(function(data) {
            dojo.forEach(data, function(currentAcc) {   
                dijit.byId("adminFirstName").setValue(currentAcc.firstName);
                dijit.byId("adminLastName").setValue(currentAcc.lastName);            
                dijit.byId("adminEmail").setValue(currentAcc.email);                                     
                if(currentAcc.dateFormat) {
                    dijit.byId("adminDateFormateWidget").set("value", currentAcc.dateFormat);
                } else {
                    dijit.byId("adminDateFormateWidget").set("value", "dd/MMM/yyyy");
                }                           
                dojo.byId("currentAdminId").value = currentAcc.id;              
            });        
        });
    },    
    update : function() {        
        var pageNode = dojo.byId("adminGeneralPage");
        var widgets = dijit.registry.findWidgets(pageNode);
        var firstNode = null;
        var state = true;
        dojo.forEach(widgets, function(el) {
        ...
        if(state === true) {           
            var firstName = dijit.byId("adminFirstName").value;
            var lastName = dijit.byId("adminLastName").value;
            var email = dijit.byId("adminEmail").value;                                    
            var currentId = dojo.byId("currentAdminId").value;
            var dateFormat = dijit.byId("adminDateFormateWidget").value;
            ...
            this._accountRestStore.put({
                ...
                dojo.forEach(result, function(resultData) {
                    if(resultData.result == "OK") {
                        ...
                        registry.byId("appToaster").show();
                        AccountSettingDetail.init();
                        AccountSettingDetail.populateValues();
//                        AccountSettingDetail.sendMail(resultData);
                    } else {
                        registry.byId("appToaster").setContent(translator.common.message.failed,"error");
                        registry.byId("appToaster").show();
                    }
    ...
    resetNewPassword : function() {
        ...
        var newPassword = dijit.byId("adminNewPassword");
        var oldPassword = dijit.byId("adminOldPassword");
        var comPassword = dijit.byId("adminConfirmPassword");
        if(state == true) {
            ...
            passwordRestStore.put({
                ...
                if(result == "OK") {
                    registry.byId("appToaster").setContent(translator.common.account.passworResetSuccess,"message");
                    registry.byId("appToaster").show();
                    dijit.byId("adminOldPassword").reset();
                    dijit.byId("adminNewPassword").reset();
                    dijit.byId("adminConfirmPassword").reset();                
                } else if(result == "OLD PASSWORD DONOT MATCH") {
                    registry.byId("appToaster").setContent(translator.common.account.oldPasswordNotMatch,"error");
                    registry.byId("appToaster").show();
                } else {
    ...
    confirmPass: function() {
        var pass =  dijit.byId("adminNewPassword").value;
        var confPass = dijit.byId("adminConfirmPassword").value;
        if(pass != confPass) {
            dijit.byId("adminConfirmPassword").validator = function() {
                dijit.byId("adminConfirmPassword").set("invalidMessage", translator.common.account.passwordNotMatch)
                return false;
            }      
...
Chart, Pie, Tom, Tooltip, MoveSlice, Default, Lines, Magnify, theme, ColumnsPlot, Highlight, connect, timing, Notification, localeCurrency, LocaleNumber) {
    ...
    notificationTimer.onStart = function() {
        ...
        if(dijit.byId("ticketNotification")) {            
            dijit.byId("ticketNotification").destroyRecursive();
        }
    ...
    },
    {
       ...
       "accountSetting": action(function() {            
            if (dijit.byId("generalForm")) {
                dijit.byId("generalForm").destroyRecursive();
            }
            if (dijit.byId("profileForm")) {
                dijit.byId("profileForm").destroyRecursive();
            }

            core.ui.loadTemplate("accountSettingPage", core.ui.getContentId());
            AccountSettingDetail.init();
            AccountSettingDetail.populateValues();
        ...
        "changePassword" : action(function () {
            core.ui.loadTemplate("passwordSettingPage", core.ui.getContentId());
        })
    });
...
window.AccountSettingDetail = AccountSettingDetail;