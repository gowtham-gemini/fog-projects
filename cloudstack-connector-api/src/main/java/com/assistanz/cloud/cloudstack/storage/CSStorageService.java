...
public class CSStorageService {
    ...
    private CreateLunOnFilerResponse getCreateLunOnFilerResponse(Document doc) {
        ...
        if (list.getLength() > 0) {
            ...
            response.setStorageIpAddress(node.getTextContent());
        }
        ...
        if (list.getLength() > 0) {
            Node node = list.item(0);
            response.setIscsiQualifiedName(node.getTextContent());
        }
        ...
        if (list.getLength() > 0) {
            Node node = list.item(0);
            response.setStoragePoolPath(node.getTextContent());
        }
    ...
    private ListLunsOnFilerResponse getListLunsOnFilerResponse(Document doc) {
        ...
        if (list.getLength() > 0) {
            Node node = list.item(0);
            response.setLunId(node.getTextContent());
        }
        ...
        if (list.getLength() > 0) {
            Node node = list.item(0);
            response.setLunIqn(node.getTextContent());
        }
        ...
        if (list.getLength() > 0) {
            Node node = list.item(0);
            response.setLunName(node.getTextContent());
        }
        ...
        if (list.getLength() > 0) {
            Node node = list.item(0);
            response.setLunVolumeId(node.getTextContent());
        }
    ...
    private AssociateLunResponse getAssociateLunResponse(Document doc) {
        ...
        if (list.getLength() > 0) {
            Node node = list.item(0);
            response.setLunIpAddress(node.getTextContent());
        }
        ...
        if (list.getLength() > 0) {
            Node node = list.item(0);
            response.setTargetIqn(node.getTextContent());
        }
