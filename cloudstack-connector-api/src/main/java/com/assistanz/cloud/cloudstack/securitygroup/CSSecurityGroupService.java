...
public class CSSecurityGroupService {
            ...
            for (int index = 0; index < list.getLength(); index++) {
                ...
                for (int childIndex = 0; childIndex < egressRuleProperties.getLength(); childIndex++) {
                    ...
                    if (property.getNodeName().equals("account")) {
                        egressRule.setAccount(property.getTextContent());
                        ...
                        egressRule.setRuleId(property.getTextContent());
        ...
        if (list.getLength() > 0) {
            ...
            for (int index = 0; index < list.getLength(); index++) {
                ...
                for (int childIndex = 0; childIndex < egressRuleProperties.getLength(); childIndex++) {
                    ...
                    if (property.getNodeName().equals("account")) {
                        ingressRule.setAccount(property.getTextContent());
                    } else if (property.getNodeName().equals("cidr")) {
                        ...
                        ingressRule.setRuleId(property.getTextContent());
                    } else if (property.getNodeName().equals("securitygroupname")) {
    ...
    private AuthorizeSecurityGroupIngressResponse getAuthorizeSecurityGroupIngressResponse(Document doc) {
        ...
        if (list.getLength() > 0) {
            Node node = list.item(0);
            response.setRuleId(node.getTextContent());
        }
     ...
     * @throws Exception
     */
    public ListSecurityGroupsResponse listSecurityGroups(HashMap<String, String> optional)
            throws Exception {
            ...
            for (int securityGroupIndex = 0; securityGroupIndex < list.getLength(); securityGroupIndex++) {
                ...
                for (int childIndex = 0; childIndex < securityGroupProperties.getLength(); childIndex++) {
                    ...
                    if (childProperty.getNodeName().equals("id")) {
                        securityGroup.setId(childProperty.getTextContent());
                    } else if (childProperty.getNodeName().equals("account")) {
                        securityGroup.setAccount(childProperty.getTextContent());
                        ...
                        securityGroup.setDomain(childProperty.getTextContent());
                    } else if (childProperty.getNodeName().equals("domainid")) {
                        securityGroup.setDomainId(childProperty.getTextContent());
                    } else if (childProperty.getNodeName().equals("name")) {
                        securityGroup.setName(childProperty.getTextContent());
                    } else if (childProperty.getNodeName().equals("project")) {
                        securityGroup.setProject(childProperty.getTextContent());
                    } else if (childProperty.getNodeName().equals("projectid")) {
                        securityGroup.setProjectId(childProperty.getTextContent());
                    ...
                    } else if (childProperty.getNodeName().equals("tags")) {
                        NodeList tagsProperties = childProperty.getChildNodes();
                        if (tagsProperties.getLength() > 0) {
                            TagsResponse tags = new TagsResponse();
                            for (int tagsIndex = 0; tagsIndex < tagsProperties.getLength(); childIndex++) {
                                Node tagsProperty = tagsProperties.item(childIndex);
