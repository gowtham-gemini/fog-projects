...
var SecurityGroup = {
    init: function() {               
       ...
       //Vpc section
        ...
        vpcRestStore.query({currentRegionId: currentRegionId}).then(function (data) {

            dojo.forEach(data, function (el) {
                vpcList.newItem({id: el.id, name: el.vpcId});
            });
            ...
            vpcWidget.placeAt("vpcImageList");
            vpcWidget.startup();
        });
...
//                            menu.startup();
        ...
        var currentRegionId = dojo.byId("selectedRegionID").value ? dojo.byId("selectedRegionID").value : "";
        securityGroupsRestStore.query({currentRegionId: currentRegionId}).then(function(data) {
    ...
    add: function() {
        ...
        if (firstNode) {
            ...
            var vpcId = dijit.byId("vpcListWidget").value;
...
var SecurityGroupRule = {
    ...
    list: function(id) {
        ...
        var dataLayout = [[
                ...
                {'name': translator.common.action, 'field': 'action',
                    'formatter': function(data) {
                        var btn = "<a class='btn-flat danger spacing' onclick='SecurityGroupRule.deleteDialogAlert(\"" + data.id + "\")'>" + translator.common.deleteData + "</a>";
                        return btn;
                    }, 'width': '100%', 'datatype': 'string', 'autoComplete': 'true'}
            ]
        ];
    ...
    populateSecurityGroupList: function(id) {
        ...
        securityGroupRestStore.query({currentRegionId: currentRegionId}).then(function(data) {
