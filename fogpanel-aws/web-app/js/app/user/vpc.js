...
//                    }   
//                    core.ui.loadTemplate("vpcHelpMenu", core.ui.getContentId()); 
//                    VPCMenuInfo.populateVPCMenus("All"); 
//                } else {
//                    core.ui.loadTemplate("vpcDashboard", core.ui.getContentId()); 
//                    VPCMenuInfo.populateVPCMenus("All");             
//                    VPCMenuInfo.init();
                ...
                "addVpc": action(function (id) {
                    ...
                    accountResourceLimitStore.query().then(function (data) {
                        dojo.forEach(data, function (limitData) {
                            if (limitData.vpcLimit !== "-1") {
                                if ((limitData.vpcUsed >= limitData.vpcLimit)) {
...
//                            if (limitData.vpcLimit !== "-1") {
//                                if ((limitData.vpcUsed >= limitData.vpcLimit)) {
//                                    dojo.byId("vpcListAddRachedLimitMsg").style.display = "block";
//                                    dojo.query("#listVpcAddButton").toggleClass("hide_text", true);
//                                } else {
//                                    dojo.byId("vpcListAddRachedLimitMsg").style.display = "none";
//                                    dojo.query("#listVpcAddButton").removeClass("hide_text", true);
//                                }
//                            } else {
//                                dojo.byId("vpcListAddRachedLimitMsg").style.display = "none";
//                                dojo.query("#listVpcAddButton").removeClass("hide_text", true);
...
//                    });
                ...
                "view": action(function (id) {
                    ...
                    if (dijit.byId("vpcTierList")) {
                        dijit.byId("vpcTierList").destroyRecursive();
                    }
                    if (dijit.byId("ipVpcTabCointainer")) {
                        dijit.byId("ipVpcTabCointainer").destroyRecursive();
                    }
                    ...
                    if (dijit.byId("webTierList")) {
                        dijit.byId("webTierList").destroyRecursive();
                    }
                    if(dijit.byId("userRouteTableList")) {
                        dijit.byId("userRouteTableList").destroyRecursive();
                    }
                    core.ui.loadTemplate("viewVpc", core.ui.getContentId());
                    ViewVpc.init(id);
                }),
            ...
            dojo.forEach(data, function (limitData) {
                if (limitData.networkLimit !== "-1") {
                    if ((limitData.networkUsed >= limitData.networkLimit)) {
                        dojo.byId("topologyTierLimitReachedDiv").style.display = "block";
                        dojo.byId("topoAddWebTierButton").style.display = "none";
                        dojo.byId("topoAddAppTierButton").style.display = "none";
                        dojo.byId("topoAddDBTierButton").style.display = "none";
                    }
                }
            });
        });
...
var VPCTierInfo = {
    ...
    'editTier': function () {
        ...
        networkRestStore.put({id: networkId.value, name: name.value, desc: desc.value, networkOffer: networkOffer.value, NWDomain: NWDomain.value, cirdchange: cirdchange.checked}).then(function (data) {
            dojo.forEach(data, function (resultData) {
                if (data && resultData.result === "OK") {
                    var vpcEditJobStat = setInterval(function () {
                        VPCTierInfo.vpcEditJob(resultData.jobId, vpcEditJobStat);
                    }, 2000);
                } else {
                    registry.byId("userToaster").setContent(translator.common.message.cloudStackError, "error");
                    registry.byId("userToaster").show();
    ...
    'deleteTier': function () {
        ...
        networkRestStore.add({tierId: currentId}).then(function (data) {
            dojo.forEach(data, function (resultData) {
                if (data && resultData.result === "OK") {
                    var vpcDeleteJobStat = setInterval(function () {
                        VPCTierInfo.deleteJobTier(resultData.jobId, resultData.tierId, vpcDeleteJobStat);
                    }, 2000);
                } else {
                    registry.byId("userToaster").setContent(translator.common.message.cloudStackError, "error");
                    registry.byId("userToaster").show();
    ...
    'restartTier': function () {
        ...
        networkRestStore.add({tierId: currentTierId, cleanup: cleanup.checked}).then(function (data) {
            dojo.forEach(data, function (resultData) {
                if (data && resultData.result === "OK") {
                    var vpcRestartJobStat = setInterval(function () {
                        ListVpc.restartJobVpc(resultData.jobId, vpcRestartJobStat);
                    }, 2000);
                } else {
                    registry.byId("userToaster").setContent(translator.common.message.cloudStackError, "error");
                    registry.byId("userToaster").show();
...
var VPCNetworkACLInfo = {
    ...
    'add': function () {
        ...
        if (firstNode) {
            ...
            networkRestStore.add({name: name, desc: desc, vpcId: vpc}).then(function (data) {
                dojo.forEach(data, function (resultData) {
                    if (data && resultData.result === "OK") {
                        var networkAclCreateJobStat = setInterval(function () {
                            VPCNetworkACLInfo.createNetworkAclJob(resultData.jobId, networkAclCreateJobStat);
                        }, 1000);
                    } else {
                        registry.byId("userToaster").setContent(translator.common.message.cloudStackError, "error");
                        registry.byId("userToaster").show();
    ...
    'deleteAcl': function () {
        ...
        networkRestStore.add({aclId: currentId}).then(function (data) {
            dojo.forEach(data, function (resultData) {
                if (data && resultData.result === "OK") {
                    var deleteNWAclJobStat = setInterval(function () {
                        VPCNetworkACLInfo.deleteJobVpc(resultData.jobId, deleteNWAclJobStat);
                    }, 2000);
                } else {
                    registry.byId("userToaster").setContent(translator.common.message.cloudStackError, "error");
                    registry.byId("userToaster").show();
    ...
    'populateTierValues': function () {
        ...
        accountResourceLimitStore.query().then(function (data) {
            dojo.forEach(data, function (limitData) {
                if (limitData.networkLimit !== "-1") {
                    if ((limitData.networkUsed >= limitData.networkLimit)) {
                        dojo.byId("vpcAddTierRachedLimitMsg").style.display = "block";
                        dojo.query("#vpcAddTierButton").toggleClass("hide_text", true);
                    } else {
                        dojo.byId("vpcAddTierRachedLimitMsg").style.display = "none";
                        dojo.query("#vpcAddTierButton").removeClass("hide_text", true);
                    }
                }
            });
        });
                ...
                {'name': translator.common.action, 'field': 'action',
                    'formatter': function (data) {
                        var vpnStatus = new FogPanel.VPNStatus({
                            onAclVPN: function () {
                                ViewVpc.replaceACLTierShow(data.referenceId);
                            },
                            ...
                            onEditVPN: function () {
                                VPCTierInfo.showEdit(data);
                            },
                            onDeleteVPN: function () {
                                VPCTierInfo.showDelete(data.referenceId);
                            },
                            forTier: true,
                            aclStat: true,
...
//        alert(agrementChecked)
            ...
            dojo.forEach(data, function (limitData) {
                if (limitData.publicIPLimit !== "-1") {
                    if ((limitData.publicIPUsed >= limitData.publicIPLimit)) {
                        dojo.byId("vpcInfoAddIPLimitReachedMsg").style.display = "block";
                        dojo.query("#vpcInfoAddIPButton").toggleClass("hide_text", true);
                    } else {
                        dojo.byId("vpcInfoAddIPLimitReachedMsg").style.display = "none";
                        dojo.query("#vpcInfoAddIPButton").removeClass("hide_text", true);
                    }
                } else {
                    dojo.byId("vpcInfoAddIPLimitReachedMsg").style.display = "none";
                    dojo.query("#vpcInfoAddIPButton").removeClass("hide_text", true);
    ...
    'deleteVpc': function () {
        ...
        networkRestStore.add({vpcId: currentId}).then(function (data) {
            dojo.forEach(data, function (resultData) {
                if (data && resultData.result === "OK") {
                    var vpcDeleteJobStat = setInterval(function () {
                        ViewVpc.deleteJobVpc(resultData.jobId, vpcDeleteJobStat);
                    }, 2000);
                } else {
                    registry.byId("userToaster").setContent(translator.common.message.cloudStackError, "error");
                    registry.byId("userToaster").show();
    ...
    'restartVpc': function () {
        ...
        networkRestStore.add({vpcId: currentId}).then(function (data) {
            dojo.forEach(data, function (resultData) {
                if (data && resultData.result === "OK") {
                    var vpcRestartJobStat = setInterval(function () {
                        ViewVpc.restartJobVpc(resultData.jobId, vpcRestartJobStat);
                    }, 2000);
                } else {
                    registry.byId("userToaster").setContent(translator.common.message.cloudStackError, "error");
                    registry.byId("userToaster").show();
    ...
    'init': function (id) {
        ...
        var viewVpc = new JsonRest({
            target: core.getContextPath() + "/api/vpc/view"
        });
        
        
        viewVpc.query({id:id}).then(function(data){
            
            dojo.byId("viewVpcReferenceId").innerHTML = data.referenceId
            dojo.byId("currentVpcId").innerHTML = data.referenceId
            dojo.byId("vpcRegionInfo").innerHTML = data.region
            dojo.byId("vpcIdInfo").innerHTML = data.id
            dojo.byId("viewVpcName").innerHTML = data.referenceId
            dojo.byId("viewVpcDhcpOptionsId").innerHTML = data.dhcpOptionsId
            dojo.byId("viewVpcInstanceTenancy").innerHTML = data.instanceTenancy
            dojo.byId("viewVpcCidrBlock").innerHTML = data.cidrBlock
            dojo.byId("viewVpcState").innerHTML = data.state
            dojo.byId("viewVpcIsDefault").innerHTML = data.isDefault
            ...
            [
                ...
                {'name': translator.common.name, 'field': 'vpcName', 'width': '200px', 'datatype': 'string', 'autoComplete': 'true', 'formatter': function (data) {
                        ...
                        if (data.state === "available") {
                            html = "<a href='#/user/vpc/view/" + data.vpcId + "'>" + data.name + "</a>";
                        } else {
                            html = data.name;
                        }
...
//                {'name': translator.common.description, 'field': 'description', 'width': '200px', 'datatype': 'string', 'autoComplete': 'true'},
                {'name': translator.common.region, 'field': 'region', 'width': '150px', 'datatype': 'string', 'autoComplete': 'true'},
                        ...
                        if (data.state === "Enabled") {
                            vpnStatus = new FogPanel.VPNStatus({
                                onConfigVPN: function () {
                                    core.router.go("#/user/vpc/view/" + data.referenceId);
                                },
                                onRebootVPN: function () {
                                    ListVpc.showRestartVpc(data.referenceId);
                                },
                                onEditVPN: function () {
                                    ListVpc.showEdit(data);
                                },
                                onDeleteVPN: function () {
                                    ListVpc.showDeleteVpc(data.referenceId);
                                }
                            });
        ...
        var vpcListRestStore = new JsonRest({
            target: core.getContextPath() + "/api/vpc/list"
        });
        
        vpcListRestStore.query({currentRegionId:currentRegionId}).then(function (data) {
            console.log(data)
            if (data.length === 0 || data === undefined || data === "undefined" || data === "" || data === " " || data === null) {
                ...
                dojo.forEach(data, function (vpcData) {
                    vpcDataList.newItem({
                        id: vpcData.vpcId,
                        vpcName: {'name': vpcData.vpcId, 'vpcId': vpcData.vpcId, state: vpcData.state},
                        description: vpcData.description,
                        region: vpcData.region,
                        cidr: vpcData.cidrBlock,
                        state: vpcData.state,
    ...
    'deleteVpc': function () {
        ...
        networkRestStore.add({vpcId: currentId}).then(function (data) {
            dojo.forEach(data, function (resultData) {
                if (data && resultData.result === "OK") {
                    var vpcDeleteJobStat = setInterval(function () {
                        ListVpc.deleteJobVpc(resultData.jobId, vpcDeleteJobStat);
                    }, 2000);
                } else {
                    registry.byId("userToaster").setContent(translator.common.message.cloudStackError, "error");
                    registry.byId("userToaster").show();
...
var AddVpc = {
    'init': function (currentId) {
        ...
        var panelZone = dojo.byId("selectedRegionID").value;
    ...
    'add': function () {
        ...
        if (firstNode) {
            ...
            networkRestStore.add({zone: zone, name: name, desc: desc, cidr: vpcCidr, vpcOffering: vpcOffering, networkDomain: networkDomain}).then(function (data) {
                dojo.forEach(data, function (resultData) {
                    if (data && resultData.result === "OK") {
                        var vpcCreateJobStat = setInterval(function () {
                            AddVpc.createVPCJob(resultData.jobId, vpcCreateJobStat);
                        }, 2000);
                        dojo.query("#addVpcCloudstackException").toggleClass("hide_text", true);
                        dojo.byId("addVpcCloudstackExceptionMsg").innerHTML = "";
                    } else {
...
var VPCMenuInfo = {    
    ...
    init: function () {
        ...
        VPCMenuInfo.populateVPCStatInfo(); 
    },
    populateVPCMenus: function (currentVPC) {
        
        
        
        var vpcMenuList = [
            {menuItemName: translator.common.vpc.home, href: "#/user/vpc/dashboard", iconClass: "icon-home", subMenu: false, dropdownIcon: "", aTagClasses: "singleMenu"},
...
//                    {subMenuItemName : translator.common.topology, href : "#/user/vpc/topology"},
                    ...
                    {subMenuItemName: translator.common.vpc.subnets, href: "#/user/tier/list"},
                    {subMenuItemName: translator.common.vpc.routeTables, href: "#/user/routeTable/list"},
                    {subMenuItemName: translator.common.vpc.internetGateways, href: "#/user/internetGateway/list"},
                ]
            },
            {menuItemName: translator.common.security, href: "#/user/vpcSecurity", iconClass: "icon-bar-chart", subMenu: true, dropdownIcon: "icon-chevron-down", aTagClasses: "dropdown-toggle",
                submenuContent: [
                    ...
                    {subMenuItemName: translator.common.vpc.securityGroups, href: "#/user/vpcSecurity/listNetworkAcl"}
                ]
            },
...
//                submenuContent: [
//                    {subMenuItemName: translator.common.vpc.customerGateways},
//                    {subMenuItemName: translator.common.vpc.virtualPrivateGateWays},
//                    {subMenuItemName: translator.common.vpnConnection},
//                ]
//            },
        ];
        var vpcOptions = {
...
//            selectBoxValue : "option",
    ...
    populateVPCStatInfo: function (currentRegionId) {
        
        if(!currentRegionId) {
            currentRegionId = dojo.byId("selectedRegionID").value ? dojo.byId("selectedRegionID").value : null;
        }
        ...
        dojo.byId("vpcCount").innerHTML = 1;
        dojo.byId("vpcTotalSubnetCount").innerHTML = 1;
        dojo.byId("vpcRouteTableCount").innerHTML = 1;
        dojo.byId("vpcTotalRunningVMCount").innerHTML = 1;
        dojo.byId("vpcElasticIpCount").innerHTML = 1;
        dojo.byId("vpcSecurityGroupCount").innerHTML = 1;
        dojo.byId("vpcNetworkAclCount").innerHTML = 1;
        dojo.byId("vpcInternetGatewayCount").innerHTML = 1;
        dojo.byId("vpcPeeringConnectionCount").innerHTML = 1;
        dojo.byId("vpcCustomerGatewayCount").innerHTML = 1;
        dojo.byId("vpcVirtualPrivateGateWayCount").innerHTML = 1;
        dojo.byId("vpcVPNConnection").innerHTML = 1;
        ...
        if (currentVPC === "option" || currentVPC === "" || currentVPC === "undefined") {
            vpcStatRestResource.query({regionId: currentRegionId}).then(function (data) {
                ...
                if (data === "undefined" || data === "" || data[0] === "undefined" || data.length === 0) {
                    ...
                    dojo.byId("vpcTotalSubnetCount").innerHTML = 0;
                    dojo.byId("vpcRouteTableCount").innerHTML = 0;
                    dojo.byId("vpcTotalRunningVMCount").innerHTML = 0;
                    dojo.byId("vpcElasticIpCount").innerHTML = 0;
                    dojo.byId("vpcSecurityGroupCount").innerHTML = 0;
                    dojo.byId("vpcNetworkAclCount").innerHTML = 0;
                    dojo.byId("vpcInternetGatewayCount").innerHTML = 0;
                    dojo.byId("vpcPeeringConnectionCount").innerHTML = 0;
                    dojo.byId("vpcCustomerGatewayCount").innerHTML = 0;
                    dojo.byId("vpcVirtualPrivateGateWayCount").innerHTML = 0;
                    dojo.byId("vpcVPNConnection").innerHTML = 0;
                } else {
                    dojo.byId("vpcCount").innerHTML = data.vpcs;
                    dojo.byId("vpcTotalSubnetCount").innerHTML = data.subnets;
                    dojo.byId("vpcRouteTableCount").innerHTML = data.routeTables;
                    dojo.byId("vpcTotalRunningVMCount").innerHTML = 0;
                    dojo.byId("vpcElasticIpCount").innerHTML = 0;
                    dojo.byId("vpcSecurityGroupCount").innerHTML = data.securityGroups;
                    dojo.byId("vpcNetworkAclCount").innerHTML = 0;
                    dojo.byId("vpcInternetGatewayCount").innerHTML = data.internetGateways;
                    dojo.byId("vpcPeeringConnectionCount").innerHTML = 0;
                    dojo.byId("vpcCustomerGatewayCount").innerHTML = 0;
                    dojo.byId("vpcVirtualPrivateGateWayCount").innerHTML = 0;
                    dojo.byId("vpcVPNConnection").innerHTML = 0;
            ...
            vpcStatRestResource.query({regionId: currentRegionId, vpcId: currentVPC}).then(function (data) {
                if (data === "undefined" || data === "" || data[0] === "undefined" || data.length === 0) {
                    dojo.byId("vpcCount").innerHTML = 0;
                    dojo.byId("vpcTotalSubnetCount").innerHTML = 0;
                    dojo.byId("vpcRouteTableCount").innerHTML = 0;
        ...
        var dataLayout = [[
                ...
                {'name': translator.common.action, 'field': 'action',
                    'formatter': function (data) {
                        var vpnStatus = new FogPanel.VPNStatus({
                            onRebootVPN: function () {
                                VPNConnection.showRestartDialog(data.referenceId);
                            },
                            onDeleteVPN: function () {
                                VPNConnection.showDeleteDialog(data.referenceId);
                            },
    ...
    'addStaticRoute': function () {
        ...
        if (firstNode) {
            ...
            networkRestStore.add({privateGatewayId: dojo.byId("currentVpcPrivateGatewayId").value, cidr: cidr.value}).then(function (data) {
                dojo.forEach(data, function (resultData) {
                    if (data && resultData.result === "OK") {
                        var createStaticRouteJobStat = setInterval(function () {
                            VPCPrivateGateway.createStaticRouteJob(resultData.jobId, createStaticRouteJobStat);
                        }, 2000);
                    } else if (resultData.result === "FAILED") {
                        registry.byId("userToaster").setContent(resultData.message, "error");
                        registry.byId("userToaster").show();
