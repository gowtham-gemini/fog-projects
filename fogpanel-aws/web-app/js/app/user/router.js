...
var RouteTable = {
    ...
    'view' : function(id) {
        var routeTableViewStore = new JsonRest({
            target: core.getContextPath() + "/api/routeTable/view"
        });
        
        routeTableViewStore.query({id:id}).then(function(data){
            
            dojo.byId("vpcInfo").innerHTML = "<a href='#/user/vpc/view/" + data.vpcId + "' title='" + translator.common.view + "'>" + data.vpcId + "</a>"
            dojo.byId("routeTableIdInfo").innerHTML = data.referenceId
        })
    ...
    'populateValues': function() {
        ...
        dojo.byId("noRouteTableMessageBox").style.display = "none";
        dojo.byId("userRouteTableList").innerHTML = "<img src='images/vmload.gif' alt='" + translator.common.loader.loading + "' height='36' width='100'/> </br> <p>" + translator.user.loader.routerLoader + "</p>";
        ...
        var dataLayout = [[
                {'name': "ID", 'field': 'id', 'width': '10%', 'datatype': 'string', 'hidden': 'true',},
                {'name': translator.common.vpc.name, 'field': 'vpc', 'width': '10%', 'datatype': 'string'},
                {'name': translator.common.name, 'field':'name', 'width': '200px', 'datatype': 'string', 'autoComplete': 'true', 'formatter': function(data) { 
                        
                        return "<a href='#/user/routeTable/view/" + data.referenceId + "' title='" + translator.common.view + "'>" + data.referenceId + "</a>";
...
//                    }, 'width': '100%', 'datatype': 'string', 'autoComplete': 'true'}
        ...
        var routeTableListStore = new JsonRest({
            target: core.getContextPath() + "/api/routeTable/list"
        });

        routeTableListStore.query({currentRegionId:currentRegionId,vpcId:currentMainVPC}).then(function (data) {                                                                
            console.log(data)
            if(data.length === 0) {
                dojo.byId("userRouteTableList").innerHTML = "";
                dojo.byId("noRouteTableMessageBox").style.display = "block";
            } else {
                dojo.byId("noRouteTableMessageBox").style.display = "none";
                ...
                dataGrid.placeAt("userRouteTableList");
                dataGrid.startup();
                dataGrid.update();
            }
        });
