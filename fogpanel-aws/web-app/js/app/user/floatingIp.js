        ...
        var dataLayout = [[
                ...
                {'name': translator.common.mappedFixedIpAddress, 'field':'mappedFixedIpAddress', 'width': '200px', 'datatype': 'string', 'autoComplete': 'true', 'formatter': function(data) { 
                        if(data.instance != null) {
                            return "<a href='#/user/server/viewServer/" + data.instance.referenceId + "' title='" + translator.common.view + "'>" + data.instance.name + ": "+ data.fixedIpAddress + "</a>";
                        } else {
                            return "-"
                        }
...
//                        status: resultData.status,
                        floatingIpPool: resultData.network.name,
                        action: resultData
                    });
                });
                dojo.byId("userFloatingIpList").innerHTML = "";
    ...
    'view': function(id) {
        ...
        networksRestStore.query({referenceId: id}).then(function (data) {
           dojo.forEach(data, function (resultData) {
              ...
              dojo.byId("floatingInstance").innerHTML = resultData.instance != null ? resultData.instance.name : "-";
