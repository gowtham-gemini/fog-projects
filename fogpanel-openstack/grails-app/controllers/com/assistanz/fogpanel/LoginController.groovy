...
class LoginController {
    ...
    def ssoSuccess = {
        ...
        String cipher = userName.substring(12);
        String cipherPassword = password.substring(12);
            
//        //decrypt session id
        BASE64Decoder decoder = new BASE64Decoder();
        
//        def decodedUserDetail = new String(userName.decodeBase64())

        String decryptedUserName = new String(decoder.decodeBuffer(cipher))
        String decryptedPassword = new String(decoder.decodeBuffer(cipherPassword))
          
//        println("******* ssoSuccess page loading ********"+params);
        println("******* ssoSuccess page loading ********"+ decryptedUserName);
        
        this.doAutoLogin(decryptedUserName, decryptedPassword, request)
        if (springSecurityService.isLoggedIn()) {
            redirect uri: SpringSecurityUtils.securityConfig.successHandler.defaultTargetUrl
        }
        else {
            redirect action: 'sso', params: params                        
        }
    }
    ...
    def sessionInfo = {
        ...
        String cipher = sessionId.substring(12);

        //decrypt session id
        BASE64Decoder decoder = new BASE64Decoder();

        String decryptedSessionId = decoder.decodeBuffer(cipher)

        println("decryptedSessionId 1: "+decryptedSessionId)
        ...
        def encryptedSessionId = encoder.encode(salt) + encoder.encode(decryptedSessionId.getBytes())

        println("encryptedSessionId 2: "+encryptedSessionId)
    ...
    def reAuth = {
        ...
        String decryptedUserName = decoder.decodeBuffer(cipher)
        
        springSecurityService.reauthenticate(decryptedUserName, null);
    }
    ...
    def verifyLink = {
        ...
        println("params.engineId"+params.engineId)
        String cipher = engineId.substring(12);

        //decrypt engine id
        BASE64Decoder decoder = new BASE64Decoder();

        String decryptedEngineId = new String(decoder.decodeBuffer(cipher))

        println("decryptedEngineId 1: "+decryptedEngineId)
//
        def validEngineId = "FOG-OS";

        if(validEngineId.equals(decryptedEngineId)) {
            item.put("result", GeneralConstants.RESULT_SUCCESS);
        } else {
    ...
    private void doAutoLogin(String username, String password, HttpServletRequest request) {
        ...
        try {
            def account = Account.findByAccountIdentifier(username)
            
           
            if(!account) {
                accountService.customRegistration(username, password);
            } else {
                // custom reset password
                if(!accountService.customResetPassword(username, password)) {
                   
                    println(" password not resetted");
            ...
            println("session1"+(session==null))
            if(session == null) {
                GrailsWebRequest webUtils = WebUtils.retrieveGrailsWebRequest()
                def httpRequest = webUtils.getCurrentRequest()  
                session = httpRequest.getSession(true)
            }
            println("session2"+(session==null))
            session.setAttribute("password", password);
            def domain = Domain.findByIsDefault(true);
            session.setAttribute("domainName", domain?.name);
            
        } catch (Exception e) {
            SCH.getContext().setAuthentication(null);
            println("Failure in autoLogin"+ e);
        }
        
    }
    /**
     * Show the sso login page of faas.
     */
    def sso = {
        println("***** sso loging****")
        def faasUrl = ApplicationHolder.getApplication().config.faas.applicationUrl
        println("faasUrl: "+faasUrl)    
        redirect(url: faasUrl)
    }
    ...
    def auth = {
        ...
        if(signupSetting == "PUBLIC") {
            ...
            def networkList = Network.findAllWhere(deleted: false)
                        
            def zenossConfig = Config.findByName(Config.ZENOSS_ENDPOINT_URL).value
            if(!regionList || !imageList || zenossConfig == "" ||  street == "" || country == "" || state == "" 
                || city == "" || zip == "" || name == "" || openstackUrl == "" || !domainList || !defaultDomain || !networkList
                || !flavorList || !zoneList) {
                enableSignup = "false";
            } 
        } else {
