...
class AccountController {
    ...
    def confirmAccount() {                
        ...
        def invitationUser = Invitation.findByName(user.username);   
        if(invitationUser) {
        ...
        if(user) {            
            ...
            if(newPassword == "" || confirPasword == "" || newPassword == " " || confirPasword == " ") {              
                ...
                if(response == "Success") {
                    ...
                    notificationService.send(user.account.email.toString(), "accountPasswordReset.subject.ftl", templateMap, "accountPasswordReset.ftl")                         
    ...
    def invitationsignup() {        
        Calendar calendar = Calendar.getInstance();                     
        def invitation = Invitation.findByName(params.invitationName);
        def logoUrl = Config.findByName(Config.ORGANISATION_BILLING_LOGO);
        if(invitation) {             
...
//        def singleSignOnUrl = ApplicationHolder.getApplication().config.openstack.singleSignOnUrl
        ...
        for(def pay: advancedPayments) {
            if(pay.isDefault == true) {
                System.out.println("Card Enabled")
               enableCard = true;
               break;
            }
        }       
