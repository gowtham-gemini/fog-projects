    ...
    def getList() {
        ...
        try{
            ...
            ArrayList<ArrayList<String>> volumeListMinimal = new ArrayList<ArrayList<String>>();
                ...
                HashMap<String,String> openStackvolumeList = new HashMap<String,String>();
                    def volumeId;
                    for (OpenStackVolume openStackVolume : openStackvolumes) {
                        
                        openStackvolumeList.put(openStackVolume.getId(),"referenceId");
                        
                        def IteratedVolume = Volume.findWhere(referenceId: openStackVolume.getId(), deleted: false, user: user)

                        if(IteratedVolume) {

                            IteratedVolume.status = openStackVolume.getStatus();
                            IteratedVolume.save(flush: true);
                            ...
                            if(openStackVolume.getAttachments().size() > 0) {
                                item.put("attachment", IteratedVolume.virtualMachine?.name + " on " + openStackVolume.getAttachments().iterator().next().getDevice());
...
//                def oldVolumes = Volume.findAllWhere(deleted: false); 
//                
//                for(def oldVolume: oldVolumes) {
//                    boolean blnExists = openStackvolumeList.containsKey(oldVolume.referenceId);
//
//                    println("blnExists"+blnExists)
//
//                    if(blnExists.toString() == "false" || blnExists == false) {
//                        oldVolume.deleted = true;
//                        oldVolume.deletedAt = new Date();
//                        oldVolume.save(flush: true);
//                    }
//                }
    ...
    def attachToInstance(requestData) {
        ...
        try { 
            ...
            VolumeAttachment volumeAttachment = os.compute().servers().attachVolume(requestData.instanceId , requestData.volumeId, null);
