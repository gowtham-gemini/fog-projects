...
//        def account = Account.findByAccountIdentifier(username)
        ...
        def invoiceList = Invoice.withCriteria {
            if(username) {
               eq("account", findByAccountIdentifier(username))  
            }
            and {
            ...
            if(invoiceList.size() != 0) {            
                for(int i=minLimit; i < maxLimit; i++) {                 
                    ...
                    if(invoice) {
                        ...
                        invoiceData.put("account", invoice.account.accountIdentifier);
   ...
   def getCurrentUsage(username) {
        ...
        def invoiceList = Invoice.withCriteria {
            if(username) {
                eq("account", Account.findByAccountIdentifier(username))  
            }
...
//                        paymentItem.put("account", payment.account.accountIdentifier);
...
//                        optional2.put("account", new String(account.accountIdentifier));
