    ...
    def generateInvoice() {
        ...
        for(def invoice: invoiceList) {
            try {
                ...
                addAviliableMonitoringAlarmItemToDraftInvoice(draftInvoice)
//                addAviliableObjectStorageToDraftInvoice(draftInvoice)                                                                
    ...
    def addAviliableFloatingIPItemToDraftInvoice(invoice) {           
        ...
        for(def floatingIP: floatingIPList) {
            try {  
                ...
                //for floating IP
                 addInvoiceItem(invoice, BillableItem.get(14), floatingIP.billingType, floatingIP.floatingIpAddress, floatingIP.referenceId, null, price, null, null)
             } catch (Exception ex) {
                 Console.print(ex); //throw ex;
             } 
        }
    ...
    def addAviliableMonitoringAlarmItemToDraftInvoice(invoice) {
        
        def alarmList = Alarm.findAllWhere(account:invoice.account, deleted:false)
        for(def alarm: alarmList) {
            try {  
                 def price = MiscellaneousOffer.get(8)?.cost        
                //for alarm
                 addInvoiceItem(invoice, BillableItem.get(16), "monthly", alarm.name, alarm.refrenceId, null, price, null, null)
             } catch (Exception ex) {
                 Console.print(ex); //throw ex;
             } 
        }
