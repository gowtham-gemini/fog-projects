...
//            }
    ...
    def getMailConfig() {
        ...
        item.put("applicationUrl", ApplicationHolder.getApplication().config.openstack.applicationUrl)
    ...
    def resourceSetupVerification(endpointUrl, adminUuid, adminPassword) {
        try {
            ...
            OSClient os = server.authenticate();
             ...
             //Openstack resource check
            KeystoneTokenV3 token = (KeystoneTokenV3) os.getAccess().getToken();
                                    
            Boolean service = false;
            Boolean computeSetup = false;
            Boolean networkSetup = false;
            Boolean volumeSetup = false;
            Boolean imageSetup = false;
            Boolean objectStoreSetup = false;
            Boolean openStackSetup = false;
            ...
            for (Catalog catalog : token.getCatalog()) {
                ...
                if(catalog.getType().equals(GeneralConstants.CATALOG_TYPE_COMPUTE_V3)) {
                    computeSetup = true;
                } else if(catalog.getType().equals(GeneralConstants.CATALOG_TYPE_NETWORK)) {
                    networkSetup = true;
                } else if(catalog.getType().equals(GeneralConstants.CATALOG_TYPE_VOLUME)) {
                    volumeSetup = true;
                } else if(catalog.getType().equals(GeneralConstants.CATALOG_TYPE_IMAGE)) {
                    imageSetup = true;
                } else if(catalog.getType().equals(GeneralConstants.CATALOG_TYPE_OBJECTSTORE)) {
                    objectStoreSetup = true;
                }
                
            }
            
            if(computeSetup == true && networkSetup == true && volumeSetup == true && imageSetup == true) {
                openStackSetup = true; 
            }
            
            System.out.println("openStackSetup: "+openStackSetup);
            if(computeSetup == false) {
                noneSetup += "nova" + ", ";
            } 
            if(networkSetup == false) {
                noneSetup += "neutron" + ", ";
            } 
            if(volumeSetup == false) {
                noneSetup += "cinder" + ", ";
            }
            if(imageSetup == false) {
                noneSetup += "glance" + ", ";
            }
            System.out.println("noneSetup; "+noneSetup);
            if(!noneSetup.isEmpty()) {
                int position = noneSetup.lastIndexOf(",");
                missingResource = noneSetup.substring(0,position);
            }
            
            
            System.out.println("missingResource: "+missingResource);
            
            item.put("openStackSetup", openStackSetup);
            item.put("missingResource", missingResource);
            item.put("objectStoreSetup", objectStoreSetup);
            response.add(item);
    ...
    def isObjectStoreEnable() {
        try {
            def objectStoreConfig = Config.findByName(Config.OBJECT_STORE_ENABLED)
            
            ArrayList<ArrayList<String>> objectStoreEnableList = new ArrayList<ArrayList<String>>();
            HashMap item = new HashMap();        

            item.put("objectStoreConfig", objectStoreConfig.value)


            objectStoreEnableList.add(item)
        
            return objectStoreEnableList;
        } catch(Exception ex){
            [ex] as JSON
        }
    }
