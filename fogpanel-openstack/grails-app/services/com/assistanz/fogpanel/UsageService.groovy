...
class UsageService {
    ...
    def usageCount() {
        ...
        usageItem.put("isNetworkEmpty", networkList ? "true" : "false");
        ...
        if(!region || !volumeTypeList || !flavorList ||  !imageList || !zoneList || !networkList || !domainList) {
            enableSignup = "false";
        } 
...
//                sendUsageFailedMail(usageFailureAccounts)
    ...
    def getSnapshotUsageData(account, startDate, serviceName, invoice) {
        ...
        // select all volume non deleted volume snap shot for this account
        
        // get time difference (volume snap last usage date - currentDate)
        
        // update the time difference value toinvoice item
        
        // change the last usage run date to current date
...
//        }
    ...
    def getObjectStoreUsageData(account, startDate, serviceName, invoice) {
