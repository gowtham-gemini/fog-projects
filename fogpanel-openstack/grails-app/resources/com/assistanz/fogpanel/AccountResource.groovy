...
//    PaymentService paymentService
    ...
    @Path("/customRegister")
    @Produces(MediaType.APPLICATION_JSON)
    def customRegister(@QueryParam("userName") String userName, @QueryParam("password") String password) {
        try {
            accountService.customRegistration(userName, password) as JSON
        } catch (Exception ex){
                [ex] as JSON
        }
    }
    ...
    String validateEmail(@QueryParam("userName") String userName) {
        try { 
            accountService.validateUser(email) as JSON;
        } catch (Exception ex) {
                [ex] as JSON
        }
    }
    ...
    @GET
    @Path("/resourceCount/byUser")
    @Produces(MediaType.APPLICATION_JSON)
    def userResourceCount() {
        try {           
            accountService.getResourceCount() as JSON 
        } catch (NullPointerException ex) {
                [ex] as JSON
        } catch (Exception ex){
                [ex] as JSON
...
//            accountService.deleteSSHKey(requestBody)as JSON;
    ...
    def pullAccountsFromOpenStack() {
        try { 
            accountService.pullAccountsFromOpenStackJobStart()
            ["OK"] as JSON;
        } catch (Exception ex) {
                [ex] as JSON
        }
    ...
    @GET
    @Path("/getResourceLimit")
    @Produces(MediaType.APPLICATION_JSON)
    def getResourceLimit(@QueryParam("accountId") String accountId) {
        try {
            accountService.getResourceLimit(accountId) as JSON
        } catch (Exception ex) {
                [ex] as JSON
        }
    }
