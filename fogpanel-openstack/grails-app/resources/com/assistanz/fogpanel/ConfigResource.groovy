...
//              zoneService.list()
    ...
    def updateOpenstackConfig(@RequestBody String requestBody) {
         try {
            ...
            println("equestData.objectStoreEnabled: "+requestData.objectStoreEnabled)
            
            Config objectStoreOptionEnable = Config.findByName(Config.OBJECT_STORE_ENABLED);
            objectStoreOptionEnable.value = requestData.objectStoreEnabled
            objectStoreOptionEnable.save(flush: true)
    ...
    def updateZenossConfig(@RequestBody String requestBody) {
         try {
            ...
            def zenossUrl = requestData.endpointUrl;
            def zenossUsernameStr = requestData.zenossUsername;
            def zenossPasswordStr = requestData.zenossPassword;
            if( zenossUrl != null &&  !zenossUrl.isEmpty() 
                && zenossUsernameStr != null &&  !zenossUsernameStr.isEmpty()
                && zenossPasswordStr != null &&  !zenossPasswordStr.isEmpty()
            ) {
                
                ZenossClient.getZenossFacadeInstance(zenossUrl, zenossUsernameStr, zenossPasswordStr);
                ZenossClient?.zenossFacade?.getCollectors();
            }
...
//        
    ...
    @GET
    @Path("/isOpenStackConfigured")
    @Produces(MediaType.APPLICATION_JSON)
    def checkIsOpenStackConfigured() {
        
        configService.checkOpenStackConfigured();
    }
    ...
    @GET
    @Path("/resourceSetupVerification")
    @Produces(MediaType.APPLICATION_JSON)
    def openstackResourceSetupVerification(@QueryParam("endpointUrl") String endpointUrl,
                                      @QueryParam("adminUuid") String adminUuid,
                                      @QueryParam("adminPassword") String adminPassword) {
            try {
               configService.resourceSetupVerification(endpointUrl, adminUuid, adminPassword) as JSON
            } catch(Exception ex){
            [ex] as JSON
        }
    ...
    @Path("/objectStore")
    @Produces(MediaType.APPLICATION_JSON)
    def objectStoreConfig() {
        configService.isObjectStoreEnable();
    }
