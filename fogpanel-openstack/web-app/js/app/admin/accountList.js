...
Chart, Pie, PlotKitGreen, Tooltip, MoveSlice, Default, Lines, Magnify, theme, ColumnsPlot, Highlight, ClusteredColumns, Legend) {
    ...
    },
    {
        ...
        "summary": action(function(id) {            
            ...
            if (dijit.byId("updateResourceLimitForm")) {
                dijit.byId("updateResourceLimitForm").destroyRecursive();
            }
        ...
        'invitation' : action(function() {            
            ...
            InvitationInfo.isConfigMissing();
        ...
        'users' : action(function(accountId) {            
            
            core.ui.loadTemplate("userList", core.ui.getContentId()); 
            UsersInfo.populateValues(accountId);
            
        }),
        "addUser" : action(function() {
            
            if (dijit.byId("userForm")) {
                dijit.byId("userForm").destroyRecursive();
            }
            if (dijit.byId("newPassword")) {
                dijit.byId("newPassword").destroyRecursive();
            }
            if (dijit.byId("confirmPassword")) {
                dijit.byId("confirmPassword").destroyRecursive();
            }
            if (dijit.byId("addUserLoader")) {
                dijit.byId("addUserLoader").destroyRecursive();
            }
            
            var accountId = "";
            accountId = dojo.byId("currentAccountId").value;
           
            core.ui.loadTemplate("userCreateForm", core.ui.getContentId()); 
            dojo.byId("usersListLink").href = "#/admin/account/users/"+accountId;
            dojo.byId("accountId").value = accountId;
            userName
        })
    ...
    isConfigMissing : function() {
        ...
        usageInfo.query().then(function(data){
            dojo.forEach(data,function (usageData) {
                if(usageData.openstackConfig == "false" || usageData.orgSetting == "false" || usageData.isFlavorEmpty === "false" || usageData.isImageEmpty === "false" || usageData.userTypeConfig === "false" || usageData.isZoneEmpty == "false") {
...
var UsersInfo = {
    ...
    'addUser' : function() {
        ...
        core.router.go("#/admin/account/addUser/"+accountId);
    },
    
    populateValues : function (accountId) {
        
        dojo.byId("currentAccountId").value = accountId;
        if(dijit.byId("userGridWidget")) {
            dijit.byId("userGridWidget").destroyRecursive();
        }
        var UserRestStore = new JsonRest({
            target: core.getContextPath()+"/api/user/list?accountId="+accountId
        });
        ...
        var gridLayout = [
            [   
                ...
                {'name': translator.common.email, 'field': 'email', 'width': '250px', datatype:"string",  autoComplete: true},  
                {'name': 'Action', 'field': 'action', 'width': '100%', 'formatter': function(data) {      
                        var htmlElement = ""
                        if(data.stat === "in progress") {
                            htmlElement = "<a class='offset4' title="+translator.common.resend+">" + "<img height='20' width='20' onclick='InvitationInfo.resetconfirmMail(\"" + data.id + "\")' src='images/popup-icons/vm_reset_icon.png'/>" + "</a>"
        ...
        UserRestStore.query().then(function(data) {            
            if(data.length === 0 || data === undefined || data === 'undefined' || data === '' || data === ' ' || data === 'null' || data === null) {
                dojo.byId("noUserMessageBox").style.display = "block";
                dojo.byId("userGridList").innerHTML = "";
            } else {
                dojo.byId("noUserMessageBox").style.display = "none";            
                dojo.forEach(data,function(user) { 
                    gridDataList.newItem({
                        ...
                        action: {id: user.id, stat: user.id}
                    });
                });
                dojo.byId("userGridList").innerHTML = "";
...
var AddUser = {
    ...
    'add': function() {
        ...
        var status = true;
        dojo.byId("userAddButtonDiv").style.display = "block";
        var pageNode = dojo.byId("userContent");
        ...
        if (firstNode) {
            ...
            var password = dijit.byId("newPassword").value;
            var accountId = dojo.byId("accountId").value;

//            dojo.style(dijit.byId("addUserLoader").closeButtonNode, "display", "none");
//            dijit.byId("addUserLoader").show();
            ...
            userRestStore.add({
                ...
                dojo.forEach(data, function(el) {
                    if (el.result === "OK") {
                        ...
                        registry.byId("appToaster").show();
                       // dijit.byId("addUserLoader").hide();
                        core.router.go("#/admin/account/users/"+accountId);
                    } else {
                        console.log("ex" + el);
                        console.log("ex msg" + el.localizedMessage);
                        registry.byId("appToaster").setContent(el.localizedMessage, "error");
                        registry.byId("appToaster").show();
                        //dijit.byId("addUserLoader").hide();
                    }
                });
    ...
    'update': function() {
        ...
        if (firstNode) {
            ...
            var userDescription = dijit.byId("userDescription").value;
            var id = dojo.byId("currentUserId").value;
    ...
    populateValues: function() {
        ...
        var recurringDataLayout = [
            [
                ...
                {'name': translator.common.amount, 'field': 'amount', 'width': '150px'},
                {'name': translator.common.billing.taxPercentage, 'field': 'taxPercentage', 'width': '100px'},
                {'name': translator.common.taxAmount, 'field': 'taxAmount', 'width': '100px'},
                {'name': translator.common.billing.grid.layout.totalAmount, 'field': 'totalAmount', 'width': '150px', 'formatter': function(data) {                         
            ...
            if(data.length == 0 || data == undefined || data == 'undefined' || data == '' || data == ' ' || data == 'null' || data == null) {
                ...
                dojo.forEach(data, function(reccuringData) {
                    recurringDataList.newItem({
                        ...
                        amount: reccuringData.amount,
                        taxPercentage: reccuringData.taxPercent,
                        taxAmount: reccuringData.taxAmount,
                        totalAmount: reccuringData.totalAmount,
...
var PyamentsInfo = {
    ...
    populateValues: function() {
        ...
        var paymentRestore = new JsonRest({
            target: core.getContextPath()+"/api/payment/"
        });
            ...
            dojo.forEach(reponseData, function(data) {                
                if(data.invoice == "yes") {
                    ...
                    var paymentDataLayout = [[
                            ...
                            {'name': translator.common.amount, 'field': 'amount', 'width': '100px'},
                            {'name': translator.common.billing.processingFee, 'field': 'processingFee', 'width': '100px'},
                            {'name': translator.common.billing.grid.layout.totalAmount, 'field': 'totalAmount', 'width': '100px', 'formatter': function(data) {                                                                      
                                    return  "<span class='orangeColor'>" + data + "</span>";
                                }
                            },
                    ...
                    paymentRestore.query({accountId: currentUserId}).then(function(data) {
                        if(data.length == 0 || data == undefined || data == 'undefined' || data == '' || data == ' ' || data == 'null' || data == null) {
                            ...
                            dojo.forEach(data, function(payData) {
                                paymentDataList.newItem({
                                    ...
                                    processingFee: payData.processingFee,
...
//                
                ...
                dojo.byId("currentAcPaid").innerHTML = data.totalPaid + " ("+data.payCount+")";
                dojo.byId("currentAcDue").innerHTML = data.totalPayable < 0  ?  0 : Math.round(data.totalPayable * 100) / 100;
                        
                dojo.byId("currentAcCreditLimit").innerHTML = data.creditLimit;
                
                var acBc = data.totalPayable < 0 ? Math.round(-1 * data.totalPayable * 100) / 100 : 0;
                
                dojo.byId("currentAcRefund").innerHTML = data.refundAmount;
                dojo.byId("currentAcBc").innerHTML =  Math.round(acBc * 100) / 100;
                dojo.byId("currentAcIncome").innerHTML = data.totalPaid;                              
            });
        });
    },
    ...
    populateValues: function() {
        ...
        var taxDataLayout = [
            [
                ...
                {'name': translator.common.billing.grid.layout.currentUsage, 'field': 'currentDue', 'width': '200px'},
                {'name':  translator.common.billing.grid.layout.totalPayable, 'field': 'totalAmount', 'width': '200px', 'formatter': function(data) {                                                                      
                        return  "<span class='orangeColor'>" + data + "</span>";
                    }
                },
    ...
    populateValues: function() {
        ...
        var gridLayout = [
            [
                ...
                {'name': translator.common.billing.currentDue, 'field': 'due', 'width': '100px', datatype:"string",  autoComplete: true, 'formatter': function(data) {                                                                      
                        return  "<span class='orangeColor'>" + data + "</span>";
                    }
                },    
...
//                },     
        ...
        this._accountListRestStore.query().then(function(data) {
            if(data == undefined || data == 'undefined' || data == '' || data == ' ' || data == 'null' || data == null || data.length == 0 ) {
                ...
                dojo.forEach(data, function(accountList) {   
                    gridDataList.newItem({
                        ...
                        due: accountList.totalPayable < 0  ?  0 + " " + accountList.currency : Math.round(accountList.totalPayable * 100) / 100 + " " + accountList.currency,
                        paid: Math.round(accountList.totalPaid * 100) / 100 + " " + accountList.currency,
                        status: accountList.status.name, 
                        userCount:accountList.userCount,
...
var ResourceLimitForUser = {
    ...
    populateValues : function(id) {
        ...
        var resourceLimitForUserRest = new JsonRest({
           target: core.getContextPath() + "/api/account/getResourceLimit/" 
        });
        ...
        resourceLimitForUserRest.query({accountId : id}).then(function(data) {
            dojo.forEach(data, function(resultData) {
                                
                console.log("resultData.snapshots"+resultData.snapshots);
                console.log("resultData.accountId"+resultData.accountId);
...
//                }
//                 if(resultData.routerLimit === "-1") {
//                    dojo.byId("PrimaryStorageLimitDiv").style.display = "none";
//                }
//                 if(resultData.subnetLimit === "-1") {
...
//                } 
                ...
                dijit.byId("instanceLimitLoader").set("value", Math.round(( resultData.instancesUsed / resultData.instances) * 100) % 100.00); 
                
                dijit.byId("volumeLimitLoader").set("value", Math.round(( resultData.volumesUsed / resultData.volumes) * 100)  % 100.00); 
                
                dijit.byId("snapshotLimitLoader").set("value", Math.round(( resultData.snapshotsUsed / resultData.snapshots) * 100)  % 100.00); 
                dijit.byId("pulicIpLimitLoader").set("value", Math.round(( resultData.publicIPUsed / resultData.publicIPLimit) * 100)  % 100.00); 
                dijit.byId("gigabytesLimitLoader").set("value", Math.round(( resultData.gigabytesUsed / resultData.gigabytes) * 100)  % 100.00);                 
                dijit.byId("securityGroupsLimitLoader").set("value", Math.round(( resultData.securityGroupsUsed / resultData.securityGroups) * 100)  % 100.00); 
//                dijit.byId("subnetLimitLoader").set("value", Math.round(( resultData.subnetLimitUsage / resultData.subnetLimit) * 100)  % 100.00); 
                dijit.byId("vcpuLimitLoader").set("value", Math.round(( resultData.coresUsed / resultData.cores) * 100)  % 100.00); 
                dijit.byId("memoryLimitLoader").set("value", Math.round(( resultData.ramUsed / resultData.ram) * 100)  % 100.00); 
                 
                var instances = resultData.instances === 0 || resultData.instances === null ? "0" : resultData.instances;
                var volumes = resultData.volumes === 0 || resultData.volumes === null ? "0" : resultData.volumes;
                var snapData = resultData.snapshots === 0 || resultData.snapshots === null ? "0" : resultData.snapshots;
                var ipData = resultData.publicIPLimit === 0 || resultData.publicIPLimit === null ? 0 : resultData.publicIPLimit;
                var gigabytesData = resultData.gigabytes === 0 || resultData.gigabytes === null ? 0 : resultData.gigabytes;
                var securityGroups = resultData.securityGroups === 0 || resultData.securityGroups === null ? 0 : resultData.securityGroups;
//                var ssData = resultData.subnetLimit === 0 || resultData.subnetLimit === null ? 0 : resultData.subnetLimit;
                var cpuData = resultData.cores === 0 || resultData.cores === null ? 0 : resultData.cores;
                var ramData = resultData.ram === 0 || resultData.ram === null ? 0 : resultData.ram;    
                
                dijit.byId("networkLimitLoader").set("value", Math.round((resultData.networkUsed / resultData.network) * 100)  % 100.00); 
                var networkData = resultData.network === 0 || resultData.network === null ? 0 : resultData.network;
                dojo.byId("networkLimitInfo").innerHTML =  resultData.networkUsed + " / " +  networkData + " Network" ;
                   
                 dojo.byId("instanceLimitInfo").innerHTML =  resultData.instancesUsed + " / " + instances + " Instance";
                 dojo.byId("volumeLimitInfo").innerHTML =  resultData.volumesUsed + " / " + volumes + " Volume";
                 dojo.byId("snapshotLimitInfo").innerHTML =  resultData.snapshotsUsed + " / " + snapData + " Snapshot";
                 dojo.byId("pulicIpInfo").innerHTML =  resultData.publicIPUsed + " / " + ipData + " IP" ;
                 dojo.byId("gigabytesLimitInfo").innerHTML =  resultData.gigabytesUsed + " / " + gigabytesData + " GB";
                 dojo.byId("securityGroupsInfo").innerHTML =  resultData.securityGroupsUsed + " / " + securityGroups + " SecurityGroup" ;
//                 dojo.byId("secondaryStorageInfo").innerHTML =  resultData.subnetLimitUsage + " / " + ssData + " GB" ;
                 dojo.byId("vcpuLimitInfo").innerHTML =  resultData.coresUsed + " / " + cpuData + " Core";
                 dojo.byId("memoryLimitInfo").innerHTML =  resultData.ramUsed + " / " +  ramData + " MB" ;
                
                
                // setting existing value 
                dijit.byId("retailMetaDataLimit").setValue(resultData.metadataItems);
                dijit.byId("retailCpuLimit").setValue(resultData.cores);
                dijit.byId("retailInstanceLimit").setValue(resultData.instances);
                dijit.byId("retailInfectedFileLimit").setValue(resultData.injectedFiles);
                dijit.byId("retailInfectedFileContentLimit").setValue(resultData.injectedFileContentBytes);
                dijit.byId("retailStorageLimit").setValue(resultData.volumes);
                dijit.byId("retailSnapshotLimit").setValue(resultData.snapshots);
                dijit.byId("retailTotalSizeOfVolumeAndSnapshot").setValue(resultData.gigabytes);
                dijit.byId("retailMemoryLimit").setValue(resultData.ram);
                dijit.byId("retailSecurityGroupLimit").setValue(resultData.securityGroups);
                dijit.byId("retailSecurityGroupRuleLimit").setValue(resultData.securityGroupRules);
                dijit.byId("retailPublicIPLimit").setValue(resultData.publicIPLimit);
                dijit.byId("retailNetworkLimit").setValue(resultData.network);
                dijit.byId("retailPortLimit").setValue(resultData.port);
                dijit.byId("routerLimit").setValue(resultData.router);
                dijit.byId("subnetLimit").setValue(resultData.subnet);
    ...
    update: function() {
        ...
        var accountId =  dojo.byId("currentAccountId").value;
        var retailInstanceLimit = Number(dijit.byId("retailInstanceLimit").value);
        var retailStorageLimit = Number(dijit.byId("retailStorageLimit").value);
        var retailSnapshotLimit = Number(dijit.byId("retailSnapshotLimit").value);
        var retailPublicIPLimit = Number(dijit.byId("retailPublicIPLimit").value);
        var retailPortLimit = Number(dijit.byId("retailPortLimit").value);
        var retailCpuLimit = Number(dijit.byId("retailCpuLimit").value);
        var retailMemoryLimit = Number(dijit.byId("retailMemoryLimit").value);
        var routerLimit = Number(dijit.byId("routerLimit").value);
        var subnetLimit = Number(dijit.byId("subnetLimit").value);
        
        var retailMetaDataLimit = Number(dijit.byId("retailMetaDataLimit").value);
        var retailInfectedFileLimit = Number(dijit.byId("retailInfectedFileLimit").value);
        var retailInfectedFileContentLimit = Number(dijit.byId("retailInfectedFileContentLimit").value);
        var retailTotalSizeOfVolumeAndSnapshot = Number(dijit.byId("retailTotalSizeOfVolumeAndSnapshot").value);
        var retailSecurityGroupLimit = Number(dijit.byId("retailSecurityGroupLimit").value);
        var retailSecurityGroupRuleLimit = Number(dijit.byId("retailSecurityGroupRuleLimit").value);
        
        
        var retailNetworkLimit = Number(dijit.byId("retailNetworkLimit").value);
        ...
        if (firstNode) {
            ...
            dojo.byId('resourceLimitBtnDiv').style.display = 'none';
            dojo.byId('resourceLimitLoader').style.display =  'block';
            updateResourceLimitRest.add({
                ...
                retailPortLimit : retailPortLimit,
                ...
                retailMetaDataLimit : retailMetaDataLimit,
                retailInfectedFileLimit : retailInfectedFileLimit,
                retailInfectedFileContentLimit : retailInfectedFileContentLimit,
                retailTotalSizeOfVolumeAndSnapshot : retailTotalSizeOfVolumeAndSnapshot,
                retailSecurityGroupLimit : retailSecurityGroupLimit,
                retailSecurityGroupRuleLimit : retailSecurityGroupRuleLimit
                
            
            }).then(function(data){
                
               dojo.byId('resourceLimitBtnDiv').style.display = 'block';
               dojo.byId('resourceLimitLoader').style.display =  'none';
