...
//                        });
                    ...
                    registry.byId('appToaster').setContent("Invalid Credentials !. Please check your entered credentials", 'error');
                    registry.byId('appToaster').show();
                }
            });
        }
...
var openstackConfig = {
    'init' : function() {
        ...
        openStackConfigCheck.query().then(function(data) {
            ...
            if(data === false) {
                dojo.byId("objectStoreConfigDiv").style.display = "none";
            } else{
                dojo.byId("objectStoreConfigDiv").style.display = "block";
            }
        });
        ...
        configRestStore.query().then(function(resultData) {
            dojo.forEach(resultData, function(config) {
                ...
                } else if (config.name === "object.store.enabled") {
                    if(config.value == "true") {
                        dojo.byId("objectStoreNotInstalledDiv").style.display = "none";
                        dojo.byId("objectStoreSelectionDiv").style.display = "block";
                        dijit.byId("objectStoreEnable").set("checked", true);
                    } else if(config.value == "false") {
                        dojo.byId("objectStoreNotInstalledDiv").style.display = "none";
                        dojo.byId("objectStoreSelectionDiv").style.display = "block";
                        dijit.byId("objectStoreDisable").set("checked", true);
                    } else {
                        dojo.byId("objectStoreNotInstalledDiv").style.display = "block";
                        dojo.byId("objectStoreSelectionDiv").style.display = "none";
    ...
    'update': function() {
        ...
        var openStackSetup = "";
        var missingResource = "";
        var objectStoreSetup = "";
        
        dojo.byId("openStackConfigButtonDiv").style.display = "none";
        dojo.byId("openStackConfigLoader").style.display = "block";
        
        var formElements = dojo.query("#objectStoreEnabledDiv input:checked");
        var checkedRadioId = dijit.byId(formElements[0].id);        
        var objectStoreSelection = dijit.byId(checkedRadioId).value;
        var objectStoreEnabled = false;
        if(objectStoreSelection == "ENABLE") {
...
//        });
        ...
        isOpenStackAuthenticated.query({
            ...
            if(data == true) {
                ...
                var resourceSetupVerification = new JsonRest({
                    target: core.getContextPath() + "/api/config/resourceSetupVerification" 
                });
                
                resourceSetupVerification.query({
                    endpointUrl : endpointUrl.getValue(),
                    adminUuid : adminUuid.getValue(),
                    adminPassword : adminPassword.getValue(),
                }).then(function(data) {
                    dojo.forEach(data, function(resultData) {
                        openStackSetup = resultData.openStackSetup;                        
                        missingResource = resultData.missingResource; 
                        objectStoreSetup = resultData.objectStoreSetup;
                        
                    });
                    
                        if(openStackSetup === true) {
                           ...
                           if(objectStoreSetup == true) {
                               
                           } else {
                               objectStoreEnabled = "notInstalled";
                           }

                           var openStackConfigRestStore = new JsonRest({
                             ...
                             if (firstNode) {
                               ...
                               openStackConfigRestStore.add({
                                     endpointUrl: endpointUrl.getValue(),
                                     adminUuid: adminUuid.getValue(),
                                   adminPassword: adminPassword.getValue(),
                                   objectStoreEnabled: objectStoreEnabled
                               }).then(function(data) {
                                     dojo.byId("openStackConfigButtonDiv").style.display = "block";
                                     dojo.byId("openStackConfigLoader").style.display = "none";
                                     if (data[0] === "OK") {
                                         registry.byId('appToaster').setContent(translator.common.message.addedSuccess, 'message');
                                         registry.byId('appToaster').show();
                                         if(objectStoreSetup == true) {
         ...
         //            });
                         ...
                         dojo.byId("openStackConfigButtonDiv").style.display = "block";
                         dojo.byId("openStackConfigLoader").style.display = "none";
                         //Openstack resource missing 
                         registry.byId('appToaster').setContent(translator.common.message.openstackSetup + " " + missingResource + " " + translator.common.message.notInstalled, 'error');
                         registry.byId('appToaster').show();
