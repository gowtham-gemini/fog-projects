...
var Instance = {
   'init': function() {
    ...
    'populateValues': function() {        
        ...
        dojo.byId("noInstanceAdminMessageBox").style.display = "none";            
        dojo.byId("adminInstanceList").innerHTML = "<img src='images/vmload.gif' alt="+translator.common.loader.imageLoadError+" height='36' width='100'/> </br> <p>"+translator.user.loader.instanceLoader+"</p>";   
        var gridData = {
            items: []
        };
        ...
        var gridLayout = [
            [                
                ...
                {'name': translator.common.userLabel, 'field': 'user', 'width': '100px', datatype:"string",  autoComplete: true},                      
                ...
                {'name': translator.common.powerState, 'field': 'powerState', 'width': '100%', 'formatter' : function(data) {                            
    ...
    serverRestStore.query().then(function(data) {
            if(data.length == 0 || data == "undefined" || data == undefined)  {                  
                dojo.byId("noInstanceAdminMessageBox").style.display = "block";                    
                dojo.byId("adminInstanceList").innerHTML = "";
            } else {                            
                dojo.byId("noInstanceAdminMessageBox").style.display = "none";
                dojo.forEach(data, function(instanceData) {  
                    gridDataList.newItem({                    
                        ...
                        powerState: instanceData.state,                                                 
                        user: instanceData.account
                    });                   
                });  
            
               if(dijit.byId("instanceAdminGridWidget")) {
                   dijit.byId("instanceAdminGridWidget").destroyRecursive();
               }              
                
                dojo.byId("adminInstanceList").innerHTML = "";
                var instanceGrid = new EnhancedGrid({
                    id: 'instanceAdminGridWidget',
                    store: gridDataList,
                ...
                instanceGrid.placeAt("adminInstanceList");        
