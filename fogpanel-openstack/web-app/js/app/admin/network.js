...
var Networks = {
    init: function() {
        ...
        openStackConfigCheck.query().then(function(data) {
            ...
            if(data === false) {
                dojo.byId("adminNetworkListDiv").style.display = "none";
                dojo.byId("openStackNotConfiguredMsgNetwork").style.display = "block";
            } else{
                Networks.populateValues();
            }
        });
...
//                {'name': "ID", 'field': 'id', 'width': '200px', 'datatype': 'string', 'autoComplete': 'true'},
                ...
                {'name': translator.common.external, 'field': 'external', 'width': '100px', 'datatype': 'string', 'autoComplete': 'true'},
...
var SubnetInfo = {
    ...
    'createFormLoad': function(id) {
        ...
        subnetsRestStore.query({referenceId: id}).then(function(data) {
            dojo.forEach(data, function(resultData) {
...
//                        return "<span class='bold'>" + "IP address: "+ data.ipAddress + " | " + "Subnet ID: "+ data.subnetId + "</span>";
    ...
    'createFormLoad': function(id) {
        ...
        portRestStore.query({referenceId: id}).then(function(data) {
            dojo.forEach(data, function(resultData) {
