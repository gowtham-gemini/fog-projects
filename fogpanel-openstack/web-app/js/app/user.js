...
var Dashboard = {
    ...
    init: function() {
        ...
        this._virtualMachineCount = new JsonRest({
            target: core.getContextPath() + "/api/virtualMachine/pieChartCount"
    ...
    loadZoneData : function() {
        var resourceCountRestStore = new JsonRest({
            target: core.getContextPath()+"/api/account/resourceCount/byUser"
        }); 
        ...
        dojo.byId("cpuCoreStatsBox").innerHTML = "";
        dojo.byId("ramStatsBox").innerHTML = "";
        dojo.byId("floatingipStatsBox").innerHTML = "";
        dojo.byId("storageStatsBoxDiv").innerHTML = "";
        dojo.byId("volumeStorageStatsBox").innerHTML = "";
        dojo.query("#userDashboardQuataLoader").removeClass("hide_text", true);
        dojo.query("#userDashboardQuataDiv").toggleClass("hide_text", true); 
        resourceCountRestStore.query().then(function(data) {
            dojo.forEach(data, function(countData) { 
                var currency = countData.currency;
                
                var vmNode = document.createElement('div');   
                vmNode.className = 'zone-1';
                vmNode.innerHTML = '<div class="zone-titles">'+ translator.common.instances +'</div><div class="zone-counts custom-stat-box"><i class="iconfg-vm_icon" ></i>'+countData.totalRunningVm + " / " + countData.vmLimit+'</div><div class="zone-pricing">'+LocaleNumber.format(countData.totalRunningVmCost, {places: 2, locale: dojo.locale})+" "+currency+'</div>';
                dojo.byId("vmStatsBox").appendChild(vmNode);                 
                
                var cpuCoreNode = document.createElement('div');   
                cpuCoreNode.className = 'zone-1';
                cpuCoreNode.innerHTML = '<div class="zone-titles">'+ translator.common.vcpus +'</div><div class="zone-counts custom-stat-box"><i class="iconosd-vcpu" ></i>' +countData.coresUsed +" / "+countData.cores+'</div><div class="zone-pricing">'+LocaleNumber.format(0, {places: 2, locale: dojo.locale})+" "+currency+'</div>';
                dojo.byId("cpuCoreStatsBox").appendChild(cpuCoreNode); 
                
                var ramNode = document.createElement('div');   
                ramNode.className = 'zone-1';
                ramNode.innerHTML = '<div class="zone-titles">'+ translator.common.raminGB +'</div><div class="zone-counts custom-stat-box"><i class="iconosd-ram" ></i>'+countData.ramUsed / 1024 + " / " +countData.ram / 1024 +'</div><div class="zone-pricing">'+LocaleNumber.format(0, {places: 2, locale: dojo.locale})+" "+currency+'</div>';
                dojo.byId("ramStatsBox").appendChild(ramNode);                 

                var floatingIPNode = document.createElement('div');   
                floatingIPNode.className = 'zone-1';
                floatingIPNode.innerHTML = '<div class="zone-titles">'+ translator.common.floatingIPs +'</div><div class="zone-counts custom-stat-box"><i class="iconosd-floating_ip" ></i>'+countData.publicIPUsed+ " / " + countData.publicIPLimit + '</div><div class="zone-pricing">'+LocaleNumber.format(countData.floatingIPCost, {places: 2, locale: dojo.locale})+" "+currency+'</div>';
                dojo.byId("floatingipStatsBox").appendChild(floatingIPNode);                                                        
                
                var volumeNodeContainer = document.createElement('div');   
                volumeNodeContainer.className = 'zone-1';
                volumeNodeContainer.innerHTML = '<div class="zone-titles">'+ translator.common.volumes +'</div><div class="zone-counts custom-stat-box"><i class="iconosd-volumes" ></i>'+countData.totalStorage + " / "+ countData.volumeLimit + '</div><div class="zone-pricing">'+LocaleNumber.format(countData.totalVolumeCost, {places: 2, locale: dojo.locale})+" "+currency+'</div>';
                dojo.byId("storageStatsBoxDiv").appendChild(volumeNodeContainer); 

                var volumeStorageNode = document.createElement('div');   
                volumeStorageNode.className = 'zone-1';
                volumeStorageNode.innerHTML = '<div class="zone-titles">'+translator.common.volumeStorageinGB+'</div><div class="zone-counts custom-stat-box"><i class="iconosd-volume_storage" ></i>'+countData.gigabytesUsed + " / "+ countData.gigabytes +'</div><div class="zone-pricing">'+LocaleNumber.format(0, {places: 2, locale: dojo.locale})+" "+currency+'</div>';
                dojo.byId("volumeStorageStatsBox").appendChild(volumeStorageNode); 
                
                
                var securityGroupNode = document.createElement('div');   
                securityGroupNode.className = 'zone-1';
                securityGroupNode.innerHTML = '<div class="zone-titles">'+ translator.common.securityGroups +'</div><div class="zone-counts custom-stat-box"><i class="iconosd-security_groups" ></i>'+countData.securityGroupsUsed + " / "+ countData.securityGroups +'</div><div class="zone-pricing">'+LocaleNumber.format(0, {places: 2, locale: dojo.locale})+" "+currency+'</div>';
                dojo.byId("securityGroupStatsBox").appendChild(securityGroupNode);        
                
//                var volumeSnapshotNode = document.createElement('div');   
//                volumeSnapshotNode.className = 'zone-1';
//                volumeSnapshotNode.innerHTML = '<div class="zone-titles">'+'Volume Snapshots'+'</div><div class="zone-counts custom-stat-box"><i class="iconfg-storage_icon" ></i>'+countData.totalVolumeSnapshot + " / "+ countData.vloumeSnapshotsLimit +'</div><div class="zone-pricing">'+LocaleNumber.format(0, {places: 2, locale: dojo.locale})+" "+currency+'</div>';
//                dojo.byId("volumeSnapshotStatsBox").appendChild(volumeSnapshotNode);                        
                
                var networkNode = document.createElement('div');   
                networkNode.className = 'zone-1';
                networkNode.innerHTML = '<div class="zone-titles">'+translator.common.networks+'</div><div class="zone-counts custom-stat-box"><i class="iconfg-netwrk_icon" ></i>'+countData.totalNetwork + " / "+ countData.network +'</div><div class="zone-pricing">'+LocaleNumber.format(countData.totalNetworkCost, {places: 2, locale: dojo.locale})+" "+currency+'</div>';
                dojo.byId("networkStatsBox").appendChild(networkNode);                                                        
            });               
            dojo.query("#userDashboardQuataLoader").toggleClass("hide_text", true);
            dojo.query("#userDashboardQuataDiv").removeClass("hide_text", true);
        });
    },
    populateValues: function() {                        
        ...
        var resourceCountRestService = new JsonRest({
            target: core.getContextPath()+"/api/public/resourceCount"
        });       
        
        resourceCountRestService.query().then(function(resourceCount) {                           
            if(resourceCount[0].privateKeyExistsInDB || resourceCount[0].privateKeyExistsInDB === "true") {
                dojo.byId("sshKeyErrorInfo").style.display = "block";                    
            } else {
                dojo.byId("sshKeyErrorInfo").style.display = "none";
            }                  
        });        
        ...
        this._accountListStore.query().then(function(data) {
             dojo.forEach(data, function(el) { 
                ...
                dojo.byId("projectName").innerHTML = el.accountName; 
                dojo.byId("projectOwnerName").innerHTML = el.userName; 
                dojo.byId("billingOwnerName").innerHTML = el.userName; 
                dojo.byId("accountUserName").innerHTML = el.userName;                 
...
//                } else {
//                    dojo.byId("chartDivMaster").style.display = "block";
//                    dojo.byId("noDataDiv").style.display = "none";
//                }                
            });                        
...
//        });
        ...
        var allNetworkMenu = [
            ...
            {menuItemName: translator.common.menu.cloud, href: "#/user/cloud/", iconClass: "icon-cloud", subMenu: true, dropdownIcon: "icon-chevron-down", aTagClasses: "dropdown-toggle",
                submenuContent : [                        
                     {subMenuItemName : translator.common.menu.servers, href : "#/user/server/list"},
            ...
            {menuItemName: translator.common.menu.virtualDataCenter, href: "#/user/virtualDataCenter/", iconClass: "index_title_icons_small_network", style: " width: 10px; height: 12px",subMenu: true, dropdownIcon: "icon-chevron-down", aTagClasses: "dropdown-toggle",
                submenuContent : [   
//                    {subMenuItemName : translator.common.menu.networkToplology, href : "#/user/vdc/dashboard"},  
             ...
             {menuItemName: translator.common.menu.objectStrore, href: "#/user/storage", iconClass: "index_title_icons_objectStore", style: " width: 10px; height: 12px",subMenu: true, dropdownIcon: "icon-chevron-down", aTagClasses: "dropdown-toggle",
                submenuContent : [   
                    {subMenuItemName : translator.common.menu.containers, href : "#/user/storage/container"}   
            ...
            {menuItemName: translator.common.menu.notificationManager, href: "#/user/notification", iconClass: "index_title_icons_cldnotification", subMenu: false, dropdownIcon: "", aTagClasses: "singleMenu"},
...
//            }             
        ...
        var configRestStore = new JsonRest({
            target: core.getContextPath()+"/api/config/objectStore"
        });
        
        var objectStoreEnable = false;
        
        configRestStore.query().then(function(resultData) {
            dojo.forEach(resultData, function(config) {

                if(config.objectStoreConfig == "true") {
                    objectStoreEnable = true;  
                } 
   
            });
            //Depending on object store config load user menu
            if(objectStoreEnable == true) {
                menuContainer = allNetworkMenu;
            } else {
                menuContainer = objectStoreDisabledMenu;
            }
