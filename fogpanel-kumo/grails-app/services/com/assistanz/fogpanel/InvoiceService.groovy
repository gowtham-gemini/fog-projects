    ...
    def generateInvoice() {
            ...
            for(def invoice: invoiceList) {
                try {
                    ...
                    // Block invoice generation for failed usage account
                    if(invoice.account.isUsageRunningFailed == false) {
            ...
            if(results == null || results == "null") {
                ...
                if(hours >= Double.parseDouble(minHour.value)) {
                    ...
                    if(stopVmBillableItem.enabled == true || stopVmBillableItem.enabled == "true") {
                        ...
                        if(ComputingOfferZoneCost.findWhere(computingOffer: computingOffer).minimumCost != 0) {
                            ...
                            newInvoiceItem.zone = computingOffer.zone
                            newInvoiceItem.usageUnitPrice = ComputingOfferZoneCost.findWhere(computingOffer: computingOffer).minimumCost
...
//    }
    ...
    def updateInvoice(invoice, account) {
            ...
            double subTotal1 = invoice.currentDue   
            invoice.subTotal1 = Math.ceil(subTotal1 * 100d) / 100d;
                    
            double subTotal2 = invoice.subTotal1;
            
            //sub total 2 calc (usage - discount)
            def discountBillableItem = InvoiceItem.findWhere(billableItem:BillableItem.get(6), invoice:invoice)
            
            if(discountBillableItem) {
                subTotal2 = invoice.subTotal1 + discountBillableItem.totalAmount
                invoice.discount = discountBillableItem.totalAmount
            } 
            invoice.subTotal2 = Math.ceil(subTotal2 * 100d) / 100d;
        
            // sub total 3 calc tax % amount + subtotal2
            Config ivaPercentage =  Config.findByName(Config.IVA_PERCENTAGE);
            double taxAmount = (invoice.subTotal2/100)* Double.parseDouble(ivaPercentage.value)
            invoice.tax = taxAmount
            double subTotal3 = invoice.subTotal2 + taxAmount
            invoice.subTotal3 = Math.ceil(subTotal3 * 100d) / 100d;
            
            // final total calc subtotal + retention % amount
            Config retentionPercentage =  Config.findByName(Config.RETENTION_PERCENTAGE);
            double retentionAmount = (invoice.subTotal3/100)* Double.parseDouble(retentionPercentage.value)
            
            invoice.retention = retentionAmount
        
            double finalTotal = (invoice.subTotal3 - retentionAmount) + (invoice.previousBalance + invoice.refundAmount) - (invoice.payment + account.credit);
            invoice.totalAmount =  Math.ceil(finalTotal * 100d) / 100d;
        ...
        for(int v=0; v < vmList.size(); v++) {
            ...
            if(computingBillableItem.enabled == true || computingBillableItem.enabled == "true") {
                ...
                if(vm.billingType == "monthly") { 
                    ...
                    double monthlyAmount = ComputingOfferZoneCost.findWhere(computingOffer: vm.computingOffer).cost * 720.00
                            
                    invoiceItem.usageUnitPrice = Math.ceil(monthlyAmount * 100d) / 100d;   
                    
                    invoiceItem.usageUnits = 1.0
                    invoiceItem.usageAmount = ComputingOfferZoneCost.findWhere(computingOffer: vm.computingOffer).cost * 720.00
                    ...
                    invoiceItem.totalAmount = Math.ceil(totalAmt * 100d) / 100d;    
                     ...
                     invoiceItem.usageUnitPrice = ComputingOfferZoneCost.findWhere(computingOffer: vm.computingOffer).cost 
                }
                invoiceItem.referenceItemName = "VirtualMachine"
                 ...
                 if(diskBillableItem.enabled == true || diskBillableItem.enabled == "true") {
                    if(volume.type == "DATADISK") {
                        ...
                        if(volume.billingType == "monthly") {
                            ...
                            double monthlyAmount = DiskOfferZoneCost.findWhere(diskOffer: volume.diskOffer).cost * 720.00 * volsize
                            ...
                            invoiceItem.usageUnitPrice = DiskOfferZoneCost.findWhere(diskOffer: volume.diskOffer).cost
                        }
