...
class DiskOfferResource {
    ...
    def create(@RequestBody String requestBody) { 
        try {                   
            ...
            newDiskOffer.zone = Zone.findByReferenceZoneId(requestData.referenceZoneId)
            newDiskOffer.cluster = Cluster.findByClusterReferenceId(requestData.clusterReferenceId)
            newDiskOffer.pod = Pod.findByPodReferenceId(requestData.podReferenceId)
            for(int i = 0; i < requestData.zoneCosts.length(); i++) {
                ...
                if(cost == 0.0) {
                    ...
                    throw new NullPointerException("minimumCost cannot be zero");
                }
    ...
    def update(@RequestBody String requestBody) {
        try {
            ...
            DiskOfferZoneCost existsDiskOfferZoneCost = DiskOfferZoneCost.findByDiskOffer(DiskOffer.get(requestData.id));
            ...
            if(existsDiskOfferZoneCost){
                ...
                def oldDiskOffer = DiskOffer.get(requestData.id)
                oldDiskOffer.diskOfferZoneCosts.clear()

                def cost = []
                cost += oldDiskOffer.diskOfferZoneCosts

                cost.each { diskOfferZoneCost ->
                    oldDiskOffer.removeFromDiskOfferZoneCosts(diskOfferZoneCost)
                }
            ...
            for(int i = 0; i < requestData.zoneCosts.length(); i++){
                ...
                diskOffer.addToDiskOfferZoneCosts(new DiskOfferZoneCost(
                        DiskOffer : diskOffer.get(requestData.id),
                        zone:Zone.get(requestData.zoneCosts[i].zoneId), 
                        cost: cost,
                        minimumCost:minimumCost))            
            }
            diskOfferService.update(diskOffer, requestData.name, requestData.description)
                ["OK"]
        } catch (ValidationException ex) {
                [ex] as JSON
        } catch (LicenseExpiredException ex) {
