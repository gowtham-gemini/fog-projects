...
require([
    ...
    "List/ListItem"],
        ...
        query, domClass, Button, LocaleNumber) {
...
var CustomItemInfo = {
    ...
    populateValues: function() {
        ...
        var customDataLayout = [[
                ...
                {'name': translator.common.taxAmount, 'field': 'taxAmount', 'width': '200px', datatype: "string", autoComplete: true},
        ...
        customRestStore.query().then(function(data) {
            dojo.forEach(data, function(resultData) {
                if (resultData.customItemList.length == 0 || resultData.customItemList == undefined || resultData.customItemList == "undefined" || resultData.customItemList == "" || resultData.customItemList == " ") {
                    ...
                    dojo.forEach(resultData.customItemList, function(cusData) {
                        ...
                        customDataList.newItem({
                            id: cusData.id,
                            taxAmount: LocaleNumber.format(cusData.taxAmount),
                            totalAmount: LocaleNumber.format(Math.round(cusData.totalAmount * 100) / 100),
                            amount: LocaleNumber.format(cusData.amount),
                ...
                dojo.forEach(data, function(countData) {
                    dojo.byId("recAmt").innerHTML = LocaleNumber.format(countData.recurringItemCost.toFixed(2));
                    dojo.byId("recCurrency").innerHTML = countData.currency;

                    dojo.byId("cusAmt").innerHTML = LocaleNumber.format(countData.customItemCost.toFixed(2));
                ...
                if (recurringItem.recurringItemList.length == 0 || recurringItem.recurringItemList == undefined || recurringItem.recurringItemList == "undefined" || recurringItem.recurringItemList == "" || recurringItem.recurringItemList == " ") {
                    ...
                    dojo.forEach(recurringItem.recurringItemList, function(recurringItemData) {
                        gridDataList.newItem({
                            ...
                            amount: LocaleNumber.format(recurringItemData.amount),
                            taxPercent: LocaleNumber.format(recurringItemData.taxPercent),
                            cycle: recurringItemData.cycle,
                            uses: recurringItemData.uses,
                            taxAmount: LocaleNumber.format(recurringItemData.taxAmount),
                            totalAmount: LocaleNumber.format(recurringItemData.totalAmount),
...
//        var pdfLink = "<a href='"+core.getBaseURL()+"/pdf/pdfLink?pdfController=pdf&pdfAction=invoiceSummary&invoiceId="+data+"&filename=invoice-"+data+"'>pdf</a>"
        ...
        var gridLayout = [
            [
                ...
                {'name': translator.common.billing.grid.layout.previousBalance, 'field': 'previousBalance', 'width': '100px', datatype: "string", autoComplete: true},
                {'name': translator.common.billing.grid.layout.payments + '(-)', 'field': 'payments', 'width': '100px', datatype: "string", autoComplete: true},
                {'name': translator.common.billing.grid.layout.currentUsage + '(+)', 'field': 'currentDue', 'width': '100px', datatype: "string", autoComplete: true, 'formatter': function(data) {
                        return  "<span class='bold'>" + data + "</span>";
                    }
                },
                ...
                {'name': translator.common.action, 'field': 'action', 'width' : '10%', 'formatter' : function(data) {
                       var pdfLink = "<a href='"+core.getBaseURL()+"/pdf/britInvoice?id="+data+"'><i class='pdf_icon'></i></div></a>";
                       return pdfLink;
                   }
               }
           ]
            ...
            if (data.length == 0 || data == undefined || data == "undefined" || data == "" || data == " ") {
                ...
                dojo.forEach(data, function(invoiceData) {
                    gridDataList.newItem({
                        ...
                        invoiceDate: invoiceData.invoiceDate, action: invoiceData.id, currentDue: LocaleNumber.format(invoiceData.currentDue)
                    });
                });
                dojo.byId("invoceListGrid").innerHTML = "";
        ...
        this._accountStore.query().then(function(data) {
            dojo.forEach(data, function(resultData) {
                ...
                if (resultData.invoice == "no") {
                    ...
                    document.getElementById("currentPdf").setAttribute("href", ""+core.getBaseURL()+"/pdf/britInvoice?id="+invoiceId);
                }                
            });
        });
    }
    ...
    updatePaymentGrid: function() {
        ...
        var gridLayout = [
            [
                ...
                {'name': translator.common.billing.grid.layout.amount, 'field': 'amount', 'width': '100px', datatype: "string", autoComplete: true},
                ...
                {'name': translator.common.billing.grid.layout.totalAmount, 'field': 'totalAmount', 'width': '200px', datatype: "string", autoComplete: true, 'formatter': function(data) {
                        return  "<span class='orangeColor'>" + data + "</span>";
                    }
                },
            ...
            if (data.length == 0 || data == undefined || data == "undefined" || data == "" || data == " ") {
                ...
                dojo.forEach(data, function(paymentData) {
                    paymentGridDataList.newItem({
                        ...
                        totalAmount: LocaleNumber.format(paymentData.totalAmount),
                        processingFee: LocaleNumber.format(paymentData.processingFee),
                        amount: LocaleNumber.format(paymentData.amount),
...
//                dojo.byId("paymentAccountName").innerHTML= resultData.userName;
                ...
                if(resultData.totalAmount <= 0) {
                    dojo.byId("currentDue").innerHTML=  0;
                    dojo.byId("creditBalance").style.display = "block";
                    var creditBalanceAmount = -1 * resultData.totalAmount;
                    dojo.byId("creditBalanceAmount").innerHTML=  LocaleNumber.format(creditBalanceAmount.toFixed(2));
                                       
                } else {
                    dojo.byId("currentDue").innerHTML=  LocaleNumber.format(resultData.totalAmount.toFixed(2));  
                ...
                dojo.byId("credit").innerHTML= LocaleNumber.format(resultData.credit.toFixed(2));
                dojo.byId("creditCurrency").innerHTML= resultData.currency;
                
                dojo.byId("Payments").innerHTML=  LocaleNumber.format(resultData.payments.toFixed(2));
