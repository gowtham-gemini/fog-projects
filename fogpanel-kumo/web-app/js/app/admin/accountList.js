...
var InfrastructureInfo = {
    ...
    populateValues: function() {
        ...
        accountListRestStore.get(currentUserId).then(function(reponseData) {
            dojo.forEach(reponseData, function(data) {
                dojo.byId("usageCost").innerHTML =  data.daily.toFixed(2);
                dojo.byId("usagePeriod").innerHTML = data.dailyDate;
                dojo.byId("currentDue").innerHTML =  data.currentDue.toFixed(2);
                dojo.byId("Payments").innerHTML =  data.payments.toFixed(2);
                dojo.byId("PaymentPeriod").innerHTML = data.paymentPeriod;
                dojo.byId("currentDuePeriod").innerHTML = data.paymentPeriod;
                
                dojo.byId("vmRunningAmount").innerHTML = data.currency +" "+ data.runningVmCost; 
                dojo.byId("vmStopAmount").innerHTML =  data.currency +" "+ data.stopVmCost;
                dojo.byId("diskAmount").innerHTML =  data.currency +" "+ data.totalDiskCost; 
                dojo.byId("snapAmount").innerHTML =  data.currency +" "+ data.totalSnapCost;
                dojo.byId("bandWidthCount").innerHTML = data.totalBandwidth + translator.common.gb;  
                dojo.byId("bandWidthAmount").innerHTML =  data.currency +" "+ data.totalBandwidthCost;
            ...
            [
                ...
                {'name':  translator.common.action, 'field': 'action',
                    'formatter': function(data) {           
                        var html = "<a href='" + core.getBaseURL() + "/pdf/britInvoice?id=" + data + "' title='"+translator.common.downloadPDF+"'><i class='pdf_icon'></i></a>" +
