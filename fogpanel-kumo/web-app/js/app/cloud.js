...
require([
    ...
    "dijit/layout/ContentPane",   
    "List/ListItem"
], function(dojo, i18n, translator, dijit, JsonRest, registry, FilteringSelect, Select,ItemFileWriteStore, DataGrid, EnhancedGrid, HorizontalSlider, domConstruct, Chart, Pie, PlotKitGreen, Tooltip, MoveSlice, Default, Lines, Magnify, theme, ColumnsPlot, Highlight, connect) {
    ...
    populateValues : function () {
        ...
        var rateCardLayout = [
            [                   
                ...
                {'name': translator.common.cost, 'field': 'cost', 'width': '100px', 'datatype':'string',  'autoComplete': 'true', "formatter": function (costData) {                        
        ...
        jobStore.add({
            ...
            dojo.forEach(data, function(resultData) {
                if(resultData.jobResult == "OK") {
                    ...
                    registry.byId("userToaster").setContent(translator.common.ip.releaseIP,"message");
                    registry.byId("userToaster").show();
                    dijit.byId("nicIpLoader").hide();
                    CurrentVMSecondaryIP.showIPForNic(dijit.byId("vmInfoAquireIPNetworkWidget").value);                
                } else if(resultData.jobResult == "Pending") {
...
var CurrentVMStorageInfo = {
    ...
    showAddVolume : function() {
        ...
        dijit.byId("diskCustomSwitch").set("value", "off");
        dojo.byId("currentVMStorageLoader").style.display = "block";  
        connect.connect(dijit.byId("diskCustomSwitch"), "onStateChanged", function(newState) {                
            CurrentVMStorageInfo.showCustomDisk(newState) // newState is "on" or "off"
        });

        connect.connect(dijit.byId("diskBillingTypeSwitch"), "onStateChanged", function(newBillTypeState) {
            CurrentVMStorageInfo.enableMonthlyCost(newBillTypeState) // newState is "on" or "off"
        });
        var configRestStoreBilling = new JsonRest({
            target: core.getContextPath()+"/api/config/"
        });
...
//        dojo.byId("volumeDiskList").style.display = "none";
        ...
        var customWidget = ""
        if(dijit.byId("diskCustomSwitch").get("value") == true) {
            customWidget = true;
        } else {
            customWidget = false;
        }
        var customWidget = ""
        if(dijit.byId("diskCustomSwitch").get("value") == "on") {
            customWidget = true;
        } else {
            customWidget = false;
        }
        ...
        var firstVal = "";
         diskOfferRestStore.query({clusterReferenceId:computClusterId, tags: tag, customized:customWidget, storageType: storageType}).then(function(data) {               
             if(data == "" || data == "" || data.length == 0) { 
                 ...
                 dojo.byId("vmDetailStorageUnit").innerHTML = "/" + translator.common.gb + translator.common.gbPerHr;   
                 ...
                 dojo.forEach(data, function(el) {
                     diskList.newItem({
                         ...
                         minSize:el.minSize,
                         maxSize: el.maxSize,
                         cluster:el.cluster.clusterReferenceId,
...
//                 dojo.byId("volumeDiskList").style.display = "block";
     ...
     createVolume : function() {        
         ...
         var checkedRadioId = dijit.byId("diskBillingTypeSwitch").get("value");        
         var billingType = "";
         if(checkedRadioId == "on") {
             billingType = "monthly";
         } else {
            billingType = "hourly";
         }
...
//        dijit.byId("vmVolumeCustomSlider").set("value", currentSpinner.value);
    ...
    getDiskData : function(currentDisk) {        
        var customWidget = "";            
        if(dijit.byId("diskCustomSwitch").value == "on") {
            customWidget = true;
        } else {
            customWidget = false;
        }
     ...
     showCustomDisk : function(customCheck) {   
        ...
        if(customStat == true || customStat == "true") {
            dojo.byId("vmDiskSizeContent").style.display = "block";
//            dojo.query("#diskOfferList").toggleClass("hide_text", true); 
        } else if(customStat == "false" || customStat== false) {
            dojo.byId("vmDiskSizeContent").style.display = "none"; 
//            dojo.query("#diskOfferList").removeClass("hide_text", true); 
        }
    },    
...
var CreateVMInfo = {
    ...
    resetTemplate : function () {    
        ...
        if (currentTemplate == "" || currentTemplate == " " || currentTemplate == undefined || currentTemplate == "undefined" || currentTemplate == null || currentTemplate == "null") {                        
            dijit.byId("instanceTempWidget").set("value", "option");                                                                        
        } else {    
            var computCost = 0;
            var avgCost = 0;
            var tempCost= 0;
            ...
            compuringOfferRestStore.query({zoneReferenceId:zoneId, templateReferenceId:tempId, storageType: storageType}).then(function(data) {                                                 
                if(data.length == 0 || data == "undefined" || data == "" ) {                                                 
                    ...
                    dojo.byId("generalAvgCost").innerHTML = "" + Math.ceil(avgCost * 100.00) / 100.00  + "/" + translator.common.month;  
                    dojo.forEach(data, function(resultData) {                            
...
//                alert(1)
        ...
        var customDiskSlider = new dijit.form.HorizontalSlider({
            ...
            onChange: function(value) { 
                ...
                diskWidget.store.fetch({query: {id: diskWidget.value},
                    onItem: function(item) {
                        if(item) {                   
                            dojo.byId("diskCost").innerHTML = value + translator.common.gb +"     |"+"        " +item.cost + "  / "+translator.common.gb + "/" + translator.common.hour;
                            dojo.byId("monthlyDiskCost").innerHTML = Math.ceil((value * item.cost * 720.00) * 100.00) / 100.00 + "/" + translator.common.month; 
                            var diskCeil = (Number(tempCost) + (Number(computCost)*720.00) + (Number(item.cost) * 720.00 * value))
                            dojo.byId("generalAvgCost").innerHTML = "" + Math.ceil(diskCeil * 100.00) / 100.00 + "/" + translator.common.month;
                        }
                    }
                }); 
            }   
    ...
    showSummary : function() {      
        ...
        if(diskWidget.value === "option" || diskWidget.value === "empty"|| diskNumValue >= 0 || diskWidget.value === "") {          
            ...
            diskWidget.store.fetch({query: {id: diskWidget.value}, onItem: function(item) {
                    if(item) {                   
                        dojo.byId("summaryDiskCost").innerHTML = " "+ item.cost + " /"+translator.common.gb + "/" + translator.common.hour;
                        dojo.byId("summaryMonthlyDisCost").innerHTML = ""+  Math.ceil((item.cost * 720.00) * 100.00) / 100.00  + " /" + translator.common.month;
                        if(item.custom == true || item.custom == "true") {
                            size = Number(customDiskSize.value);
                            dojo.byId("summaryDiskSize").innerHTML = customDiskSize.value + "  " + translator.common.gb;
                            dojo.byId("summaryMonthlyDisCost").innerHTML = ""+  Math.ceil((size * item.cost * 720.00) * 100.00) / 100.00  + "   /" + translator.common.month;                                      
                        } else {
            ...
            computWidget.store.fetch({query: {id: computWidget.value},
                onItem: function(item) {
                    if(item) { 
                        ...
                        dojo.byId("summaryBandwidth").innerHTML = item.bandwidth + " " + translator.common.gb;
                        dojo.byId("summarySetupCost").innerHTML = ""+item.setupCost;
                        dojo.byId("summaryRunningCost").innerHTML =""+ item.cost +  "   /" + translator.common.hour;            
                        dojo.byId("summaryMonthlyComCost").innerHTML = ""+  Math.ceil((item.cost * 720.00) * 100.00) / 100.00  + "  /" + translator.common.month;
        ...
        tempWidget.store.fetch({query: {id: tempWidget.value},
            onItem: function(item) {
                if(item) {                   
                    ...
                    dojo.byId("summaryTempCost").innerHTML = "" + item.cost + "   /" + translator.common.month;
                    tempCost = Number(item.cost);                           
                }
            }
        }); 
        dojo.byId("summaryAvgCost").innerHTML = "" + Math.ceil(diskCost + computCost +  tempCost).toFixed(2) + "   /" + translator.common.month; 
    ...
    getDisk : function(currentDisk) {          
        ...
        if(currentDisk.value == "option") {            
            ...
            dojo.byId("generalAvgCost").innerHTML = "" +  Math.ceil(totCost * 100.00) / 100.00 + "   /" + translator.common.month;
        } else {
            ...
            currentDisk.store.fetch({query: {id: currentDisk.value},        
                onItem: function(item) {
                    if(item) {                                       
                        ...
                        if(item.custom == true || item.custom == "true") {
                            dojo.byId("diskCost").innerHTML = " 1 GB   |"+"        " +item.cost + "  /" + translator.common.gb + "/" + translator.common.month;
                            dojo.byId("monthlyDiskCost").innerHTML = Math.ceil((item.cost * 720.00) * 100.00) / 100.00 + " /" + translator.common.month; 
                            avgCost =  Number(tempCost) + Number(computCost) * 720.00 +  Number(item.cost) * 720.00 * 1;
                            dojo.byId("generalAvgCost").innerHTML = "" + Math.ceil((avgCost * 100) / 100) + " /" + translator.common.month;
                        } else if(item.custom == false || item.custom == "false") {
                            dojo.byId("diskCost").innerHTML = item.size + "  " + translator.common.gb +  "  |"+"         " +item.cost + "  /" + translator.common.gb + "/" + translator.common.hour;
                            dojo.byId("monthlyDiskCost").innerHTML = Math.ceil((item.size * item.cost * 720.00) * 100.00) / 100.00 + "  /" + translator.common.gb + "/" + translator.common.month; 
...
//            }                
            ...
            dojo.query("#storageDiskOfferContent .updatable").removeClass("non_updatable", true);                
            if(customCheck.checked == true || customCheck.checked == "true") {
                diskCost = data[0].diskOfferZoneCosts[0].cost * data[0].minSize;
                dojo.byId("diskCost").innerHTML = "  1" + translator.common.gb +"|     " +  data[0].diskOfferZoneCosts[0].cost + "  /" + translator.common.gb + "/" + translator.common.hour;    
                dojo.byId("vmDiskSize").style.display = "block";
                dojo.query("#offeringInfo #diskOfferInfo").toggleClass("show_lable", true);        
                ...
                dojo.byId("diskCost").innerHTML = data[0].size + "  GB  " +  data[0].diskOfferZoneCosts[0].cost + "  /" + translator.common.gb + "/" + translator.common.hour;
                dojo.byId("vmDiskSize").style.display = "none";                     
                dojo.query("#offeringInfo #diskOfferInfo").removeClass("show_lable", true); 
                avgCost = Number(tempCost) + Number(computCost) * 720 +  0;
                dojo.byId("generalAvgCost").innerHTML = "    " + avgCost.toFixed(2) + "   /"+translator.common.month;
                dojo.forEach(data, function(el) {
...
viewOptionsByZone : function(currentZone) {
    ...
    if(zoneId == "option") {  
         ...
         var noTempGroupData = {
            identifier: 'id',
            label: 'name',
            items: [{id:"option", name: translator.common.message.noTemplate}]
        };
...
getComputingOffer :function(template) {    
    ...
    template.store.fetch({query: {id: template.value},
        onItem: function(item) {
            if(item.id != "option") {                      
                ...
                dojo.query("#offeringInfo #vmAvgCostInfo").removeClass("show_lable", true);
                dijit.byId("customCheckBox").set("disabled", false);                
                dojo.byId("tempCost").innerHTML = item.name + "    |    "+item.cost + "      /" + translator.common.month;
    ...
    var checkedRadioId = dijit.byId(formElements[0].id);        
    var storageType = dijit.byId(checkedRadioId).value;            
    if(template.value == "option" || template.value == "empty") {             
    } else {              
    ...
    currentWidget.store.fetch({query: {id: currentWidget.value},
        onItem: function(item) {
            ...
            if(item.id == "option" || item.id == "empty" || idNum >= 0) {
                ...
                dojo.byId("computCost").innerHTML = item.core + "x" + item.memory + " MB |        " + item.cost + " /" + translator.common.hour;    
                dojo.byId("monthlyComputCost").innerHTML = Math.ceil((item.cost * 720.00) * 100.00) / 100.00 + " /" + translator.common.month;
            }                                      
        }
    });
        ...
        diskOfferRestStore.query({clusterReferenceId:clusterId, tags: type, customized:customWidget, storageType: storageType}).then(function(data) {                 
            if(data == "" || data == "") {
                ...
                if(customWidget == false) {
                    ...
                    dojo.byId("generalAvgCost").innerHTML = "" + Math.ceil((costCeil * 100.00) / 100.00) + " /" + translator.common.month;
                    dojo.byId("diskOfferCost").value = data[0].diskOfferZoneCosts[0].cost;                            
                    var firstValue = ""
...
add: function() {     
    ...
    if (vmNameWidget.validate && !vmNameWidget.validate()) {
        vmNameWidget.focus();                                     
    } else if(zoneWidget.value == "option" || zoneWidget.value == "empty" || tempWidget.value == "option" || tempWidget.value == "empty" ||  computWidget.value == "option" || computWidget.value == "empty" || compuNumValue >= 0) {
        ...
        this._instanceRestStore.query().then(function(data) {
            if(data.length == 0 || data == "undefined" || data == undefined)  {                  
                ...
                gridDataList.newItem({
                    ...
                    vmName: instanceData.name + "," + instanceData.referenceId + "," + instanceData.osCategory + "," + instanceData.id,
            ...
            instanceRestStore.query().then(function(data) {
                dojo.forEach(data, function(vmData) {                   
                    instanceDataList.newItem({                        
                        ...
                        vmName:vmData.name + "," + vmData.referenceId + "," + vmData.osCategory + "," + vmData.id,
...
//        dojo.byId("instancePlanTempSize").innerHTML = "";
        ...
        computingOfferRestStore.query({clusterReferenceId:clusterId, tags:tags, baseOs: baseOs, type:type, templateReferenceId:templateReferenceId}).then(function(data) {            
            var firstVal = "";
            dojo.forEach(data, function(el) {  
                ...
                computList.newItem({
                    ...
                    cost:el.computingOfferZoneCosts[0].cost,
                    setupCost:el.computingOfferZoneCosts[0].setupCost,
...
var AddStorageInfo = {
                ...
                if(this.value == "option") {
                    ...
                    diskOfferRestStore.query({clusterReferenceId:clusterId, tags: type, customized:customWidget, storageType: storageType}).then(function(data) {       
                        if(data == "" || data == "") {                     
                            ...
                            dojo.byId("addDiskInfo").style.display = "block";
                            dojo.byId("storageDiskCost").innerHTML = "" + data[0].diskOfferZoneCosts[0].cost + " ";
                            dojo.byId("storageDiskSize").innerHTML = data[0].size + translator.common.gb;
                        
                            var firstDisk = data[0].diskOfferReferenceId;
                        
                            dojo.byId("storageMonthlyCost").innerHTML = Math.ceil((data[0].size * data[0].diskOfferZoneCosts[0].cost * 720.00) * 100.00) / 100.00 + " /" + translator.common.month; 
            ...
            if(currentDisk.value == "option") {            
               ...
               currentDisk.store.fetch({query: {id: currentDisk.value}, onItem: function(item) {
                       if(item) { 
                           ...
                           dojo.byId("storageDiskCost").innerHTML = "" + item.cost + " ";
                           if(item.custom == true || item.custom == "true") {
                               dojo.byId("storageDiskSize").innerHTML = dijit.byId("storageDiskSizeSpinner").value + translator.common.gb;                        
                                dojo.byId("storageMonthlyCost").innerHTML = Math.ceil((dijit.byId("storageDiskSizeSpinner").value * item.cost * 720.00) * 100.00) / 100.00 + " /" + translator.common.month;                         
                            } else if(item.custom == false || item.custom == "false") {
                                dojo.byId("storageDiskSize").innerHTML = item.size + translator.common.gb;                        
                                dojo.byId("storageMonthlyCost").innerHTML = Math.ceil((item.size * item.cost * 720.00) * 100.00) / 100.00 + " /" + translator.common.month; 
                            }                 
                        }          
                    }
                }); 
    ...
    diskOfferRestStore.query({clusterReferenceId:clusterId,tags: type, customized:customCheck.checked, storageType: storageType}).then(function(data) {        
        if(data == "" || data == "" || data == undefined || data == "undefined") {      
            ...
            dojo.byId("addDiskInfo").style.display = "block";                        
            dojo.byId("storageDiskCost").innerHTML = data[0].diskOfferZoneCosts[0].cost;            
            dojo.byId("storageMonthlyCost").innerHTML = Math.ceil((1* Number(data[0].diskOfferZoneCosts[0].cost) * 720.00) * 100.00) / 100.00 + " /" + translator.common.month;
            dojo.forEach(data, function(el) {
                    diskDataList.newItem({
                        id:el.diskOfferReferenceId,
                        name: el.name,
